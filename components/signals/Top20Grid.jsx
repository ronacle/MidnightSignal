    ...
    <div className={`panel stack premium-board-shell ${experience.boardCardStyle}`}>
      ...
      <div className={`top20-grid ${experience.boardColumnsClass}`}>
        {visibleAssets.map((asset, index) => {
          ...
          const liveToneClass = liveTag?.tone ? ` live-tone-${liveTag.tone}` : '';
          return (
          <button key={asset.symbol} type="button" className={`top20-card premium-top20-card ${experience.boardCardStyle} ${state.selectedAsset === asset.symbol ? 'active' : ''} ${pulseEnabled ? 'is-live-scan' : ''} ${index === 0 ? 'is-scan-lead' : ''}${liveToneClass}`} onClick={() => { setState((prev) => ({ ...prev, selectedAsset: asset.symbol, watchlist: prev.watchlist.includes(asset.symbol) ? [asset.symbol, ...prev.watchlist.filter((item) => item !== asset.symbol)] : prev.watchlist })); onAssetOpen?.(asset); }}>
            <div className="row space-between"><div><div className="asset-name">{asset.symbol}</div><div className="asset-meta">{asset.name}</div></div><div className="top20-card-status">{liveTag ? <span className={`top20-live-tag tone-${liveTag.tone || 'steady'}`}>{liveTag.label}</span> : null}<div className={`sentiment ${asset.sentiment}`}>{asset.sentiment}</div></div></div>
