...
export default function Dashboard() {
  ...
  async function refreshMarket() {
    ...
    try {
      const params = new URLSearchParams({
        mode,
        currency,
        previousSymbol: lastTop?.symbol || '',
        previousConfidence: String(lastTop?.confidence ?? '')
      });

      const response = await fetch('/api/signals/live?' + params.toString(), { cache: 'no-store' });
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data.snapshot) throw new Error(data.error || 'Live signal refresh failed');

      const next = data.snapshot as TrustSnapshot;
      setSnapshot(next);
      ...
      const fallbackSignals = buildSignals(mode);
      setSnapshot({
        signals: fallbackSignals,
        source: 'Fallback demo data',
        updatedAt: new Date().toISOString(),
        marketCondition: 'active',
        confidenceReason: `${fallbackSignals[0].symbol} is using fallback demo data because the live market feed was unavailable.`
      });
    } finally {
  ...
  useEffect(() => {
    const id = window.setInterval(() => refreshMarket(), 5 * 60 * 1000);
    return () => window.clearInterval(id);
  }, [mode, currency, authUser?.id, lastTop?.symbol, lastTop?.confidence]);
