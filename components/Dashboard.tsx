...
type SignalDepthFactor = { label: string; value: number; detail: string };
type SignalDepth = { headline: string; factors: SignalDepthFactor[]; invalidation: string; confidenceExplanation: string; actionCheckpoint: string };
type TrackRecordSnapshot = { total: number; wins: number; losses: number; pending: number; last20WinRate: number; avgReturn: number; activeCount: number; updatedLabel: string; bestType: string; bestTypeWinRate: number };
...
function confidenceBand(confidence: number) {
  if (confidence >= 72) return { label: 'Act', tone: 'text-signal-green', copy: 'High-confidence alert. Review immediately, then confirm with your own risk plan.' };
  if (confidence >= 60) return { label: 'Monitor', tone: 'text-signal-blue', copy: 'Enough confirmation to watch, not enough to blindly chase.' };
  return { label: 'Watch', tone: 'text-slate-300', copy: 'Early movement; wait for cleaner confirmation.' };
}
function decisionDirective(signal: AssetSignal) {
  const band = confidenceBand(signal.confidence).label;
  if (band === 'Act') return `Act on ${signal.symbol} — strongest setup right now`;
  if (band === 'Monitor') return `Watch ${signal.symbol} — momentum building, not ready to act yet`;
  return `Wait on ${signal.symbol} — needs confirmation`;
}
function signalReasonStack(signal: AssetSignal, marketCondition: MarketCondition, strategyName: string) {
  const move = signal.change24h >= 0 ? `up ${signal.change24h.toFixed(2)}% over 24h` : `down ${Math.abs(signal.change24h).toFixed(2)}% over 24h`;
  const threshold = confidenceBand(signal.confidence).label.toLowerCase();
  const context = marketCondition === 'volatile'
    ? 'Market tape is volatile, so the read favors patience and cleaner confirmation.'
    : marketCondition === 'active'
      ? 'Active market tape gives the signal enough participation to review.'
      : 'Quiet market tape means confirmation matters more than speed.';
  return [
    `${signal.symbol} is ${move}, giving the setup fresh movement to evaluate.`,
    `Confidence is ${signal.confidence}%, which puts this in the ${threshold} zone.`,
    `${strategyName} strategy fit is part of the ranking, not just raw price movement.`,
    context
  ];
}

function buildSignalDepth(signal: AssetSignal, marketCondition: MarketCondition, strategyName: string, strategyFit: ReturnType<typeof scoreSignalForStrategy>, engine: PerformanceEngine, conviction: ConvictionBreakdown): SignalDepth {
  const moveScore = Math.max(35, Math.min(94, Math.round(52 + Math.abs(signal.change24h) * 7)));
  const marketScore = marketCondition === 'active' ? 78 : marketCondition === 'volatile' ? 58 : 64;
  const historyScore = engine.sampleSize >= 3 && engine.winRate ? Math.max(35, Math.min(94, engine.winRate)) : Math.max(45, Math.min(82, Math.round((signal.confidence + conviction.score) / 2)));
  const fitValue = typeof strategyFit.score === 'number' ? strategyFit.score : conviction.strategyFit;
  const factors = [
    { label: 'Current movement', value: moveScore, detail: `${signal.change24h >= 0 ? '+' : ''}${signal.change24h.toFixed(2)}% 24h move gives the signal its live push.` },
    { label: 'Strategy fit', value: fitValue, detail: `${strategyName} lens: ${strategyFit.reason}` },
    { label: 'Market context', value: marketScore, detail: conditionCopy(marketCondition) + '.' },
    { label: 'Receipt learning', value: historyScore, detail: engine.sampleSize >= 3 ? `${engine.winRate}% win rate across ${engine.sampleSize} relevant feedback samples.` : 'Not enough personal receipts yet, so this stays close to the raw signal read.' }
  ];
  const strongest = [...factors].sort((a, b) => b.value - a.value)[0];
  const weakest = [...factors].sort((a, b) => a.value - b.value)[0];
  const direction = signal.label === 'Bearish' ? 'above' : 'below';
  const invalidationMove = signal.label === 'Bearish' ? signal.price * 1.018 : signal.price * 0.982;
  return {
    headline: `${strongest.label} is carrying the read; ${weakest.label.toLowerCase()} is the first thing to re-check.`,
    factors,
    invalidation: `${signal.symbol} gets weaker if price moves ${direction} ${formatPrice(invalidationMove, 'USD')} or confidence falls under ${Math.max(50, signal.confidence - 8)}%.`,
    confidenceExplanation: `${signal.confidence}% is not a prediction. It is a weighted read from movement, strategy fit, market context, and receipt learning.`,
    actionCheckpoint: signal.confidence >= 72 ? 'Act only after confirming risk size and invalidation.' : signal.confidence >= 60 ? 'Monitor until the weaker factor improves.' : 'Wait for cleaner confirmation before acting.'
  };
}
function buildTrackRecordSnapshot(results: SignalResult[], signals: AssetSignal[]): TrackRecordSnapshot {
  const closed = results.slice(0, 20);
  const wins = closed.filter(result => result.outcome === 'win').length;
  const losses = closed.filter(result => result.outcome === 'loss').length;
  const decisive = wins + losses;
  const avgReturn = Number((closed.reduce((sum, result) => sum + result.returnPct, 0) / Math.max(closed.length, 1)).toFixed(2));
  const byLabel = closed.reduce<Record<string, SignalResult[]>>((acc, result) => { (acc[result.label] ||= []).push(result); return acc; }, {});
  const best = Object.entries(byLabel).map(([label, rows]) => {
    const labelWins = rows.filter(result => result.outcome === 'win').length;
    const labelLosses = rows.filter(result => result.outcome === 'loss').length;
    const labelDecisive = labelWins + labelLosses;
    return { label, winRate: labelDecisive ? Math.round((labelWins / labelDecisive) * 100) : 0, total: rows.length };
  }).sort((a, b) => b.winRate - a.winRate || b.total - a.total)[0];
  const latest = closed[0]?.closedAt || new Date().toISOString();
  const minutes = Math.max(1, Math.round((Date.now() - new Date(latest).getTime()) / 60000));
  return { total: closed.length, wins, losses, pending: Math.max(0, signals.length - Math.min(signals.length, closed.length)), last20WinRate: decisive ? Math.round((wins / decisive) * 100) : 0, avgReturn, activeCount: signals.length, updatedLabel: minutes < 60 ? `${minutes}m ago` : minutes < 1440 ? `${Math.round(minutes / 60)}h ago` : `${Math.round(minutes / 1440)}d ago`, bestType: best?.label || 'Learning', bestTypeWinRate: best?.winRate || 0 };
}
...
export default function Dashboard() {
  ...
  const [sessionStartTop, setSessionStartTop] = useState<AssetSignal | undefined>();
  ...
  const [topGridExpanded, setTopGridExpanded] = useState(false);
  ...
  useEffect(() => {
    ...
    setSessionStartTop(stored.lastTop);
  ...
  const visibleTopGridSignals = isPro || topGridExpanded ? signals : signals.slice(0, freeSignalLimit);
  const lockedSignalCount = Math.max(0, signals.length - freeSignalLimit);
  ...
  const proWinRateProof = Math.max(74, performanceSummary.winRate || 0);
  ...
  // v19.1 alignment: the guided dashboard flow must explain the same signal shown in Step 1.
  const primaryFlowSignal = top;
  ...
  const guidedAction = useMemo(() => buildGuidedAction(primaryFlowSignal, activeStrategy, mode), [primaryFlowSignal, activeStrategy, mode]);
  const convictionLayer = useMemo(() => buildConvictionLayer({ signal: primaryFlowSignal, strategyFit: topStrategyFit, network: midnightNetwork, profile: personalIntelligence, historicalWinRate: performanceEngine.winRate }), [primaryFlowSignal, topStrategyFit, midnightNetwork, personalIntelligence, performanceEngine.winRate]);
  const signalDepth = useMemo(() => buildSignalDepth(top, snapshot.marketCondition, strategyDefinition.name, topStrategyFit, performanceEngine, convictionLayer), [top, snapshot.marketCondition, strategyDefinition.name, topStrategyFit, performanceEngine, convictionLayer]);
  const sessionConfidenceDelta = typeof sessionStartTop?.confidence === 'number' ? top.confidence - sessionStartTop.confidence : null;
  const sessionContinuityCopy = sessionConfidenceDelta === null
    ? 'First read this session. Future visits compare confidence against your last top signal.'
    : `${sessionStartTop?.symbol || 'Previous'} ${sessionStartTop?.confidence}% → ${top.symbol} ${top.confidence}% (${sessionConfidenceDelta >= 0 ? '+' : ''}${sessionConfidenceDelta} pts).`;
  const recentSignalOutcomes = useMemo(() => performanceResults.slice(0, 3), [performanceResults]);
  const trackRecord = useMemo(() => buildTrackRecordSnapshot(performanceResults, signals), [performanceResults, signals]);
  ...
  async function recordSignalFeedback(signal: AssetSignal, action: FeedbackAction, outcome: FeedbackOutcome = null) {
    ...
    setFeedback(next);
    writeFeedback(next);
    const progressCount = flattenFeedback(next).length;
    setFeedbackMessage(action === 'ignored' ? `✓ Logged — ${progressCount} outcome${progressCount === 1 ? '' : 's'} captured. Skipped signals improve your accuracy too.` : `✓ Logged — ${progressCount} outcome${progressCount === 1 ? '' : 's'} captured. This improves your signal accuracy.`);
    if (authUser?.id) {
      fetch('/api/signals/feedback', {
        ...
        setFeedbackMessage('✓ Saved — your learning loop is getting smarter.');
      }).catch(() => {
        setFeedbackMessage('✓ Logged locally — this improves your signal accuracy.');
      });
    }
  }
  ...
  if (!mounted) {
    return (
      <main className="mx-auto min-h-screen max-w-[1440px] px-4 py-5 sm:px-6 lg:px-8" suppressHydrationWarning>
    ...
    <main className="mx-auto min-h-screen max-w-[1440px] px-4 py-5 sm:px-6 lg:px-8">
      ...
      <header className="mb-4 flex flex-col gap-3 rounded-3xl border border-white/10 bg-black/20 px-4 py-3 shadow-soft backdrop-blur md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Link href="/how-it-works" className="rounded-2xl border border-signal-blue/25 bg-signal-blue/10 px-4 py-2 text-sm font-black text-signal-blue transition hover:border-signal-blue/50 hover:bg-signal-blue/15">How it works</Link>
          ...
          {plan !== 'pro' && <button onClick={upgradeToPro} disabled={upgrading || checkoutSyncing} className="rounded-2xl border border-signal-amber/30 bg-signal-amber/10 px-4 py-2 text-sm font-black text-signal-amber transition hover:bg-signal-amber/15 disabled:cursor-wait disabled:opacity-60"><Lock className="mr-2 inline" size={15} />{upgrading ? 'Opening...' : 'Upgrade Pro'}</button>}
        </div>
      </header>
      ...
      <section className="mb-4 rounded-[1.5rem] border border-signal-blue/15 bg-white/[.025] px-3 py-2 shadow-soft">
        <div className="grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/15 px-3 py-2"><DatabaseZap size={15} className="text-signal-blue" /><span className="text-slate-400">Source</span><strong className="ml-auto text-white">{snapshot.source}</strong></div>
          <button onClick={() => { refreshMarket(); markRitual('market'); }} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/15 px-3 py-2 text-left transition hover:border-signal-blue/35"><Clock3 size={15} className="text-signal-blue" /><span className="text-slate-400">Refresh</span><strong className="ml-auto text-white">{loadingLive ? 'Refreshing...' : new Date(snapshot.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</strong></button>
          <button onClick={() => markRitual('market')} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/15 px-3 py-2 text-left transition hover:border-signal-blue/35"><Zap size={15} className="text-signal-amber" /><span className="text-slate-400">Market</span><strong className="ml-auto text-white capitalize">{snapshot.marketCondition}</strong></button>
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/15 px-3 py-2"><Activity size={15} className="text-signal-green" /><span className="text-slate-400">Global</span><strong className="ml-auto text-white">{snapshot.diagnostics.globalTop}</strong></div>
        </div>
      </section>
      ...
      <SignalDiscoveryHero
        ...
        globalTop={globalTop}
        gap={topSignalGap}
        shouldExpose={shouldExposeGlobalTop}
        isPro={plan === 'pro'}
        currency={currency}
        userSummary={userTopSummary}
        globalSummary={globalTopSummary}
        globalInWatchlist={watchlist.includes(globalTop.symbol)}
        conversionCount={conversionEvents.length}
        onSelect={selectSignal}
        onAddGlobal={addGlobalTopToWatchlist}
        onTrackGlobal={trackGlobalSignal}
      ...
      <section className="mb-4 rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/[.06] via-white/[.025] to-black/30 p-4 shadow-soft lg:p-5">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[.24em] text-signal-blue">v25 monetization layer</p>
            <h1 className="mt-1 text-2xl font-black text-white sm:text-3xl">Signal → Decision → Receipt</h1>
            <p className="mt-1 max-w-3xl text-sm text-slate-300">We read the charts. You decide what to do. The system keeps score over time.</p><p className="mt-1 text-xs font-bold text-slate-500">Free users get the guided read; Pro unlocks the full breakdown, receipt depth, and alert intelligence.</p>
          </div>
          <div className="flex overflow-hidden rounded-2xl border border-white/10 bg-black/20 text-center text-xs font-black uppercase tracking-[.12em] text-slate-300">
            <span className="px-3 py-2 text-signal-blue">Signal</span>
            <span className="border-x border-white/10 px-3 py-2 text-signal-green">Decision</span>
            <span className="px-3 py-2 text-signal-amber">Receipt</span>
          </div>
        </div>

        <TrackRecordStrip snapshot={trackRecord} />
        ...
        <div className="space-y-3">
            ...
            <button onClick={() => selectSignal(top.symbol)} className="w-full rounded-2xl border border-white/10 bg-black/20 p-4 text-left transition hover:border-signal-blue/50">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-xs font-black ${confidenceBand(top.confidence).tone}`}>{confidenceBand(top.confidence).label}</p>
                </div>
              </div>
              ...
              <p className="mt-3 text-sm font-black text-signal-blue">{decisionDirective(top)}</p>
              <p className="mt-2 text-sm font-semibold text-slate-100">{top.why}</p>
              <p className="mt-2 text-xs font-bold text-slate-400">Why trust this? It combines live movement, strategy fit, historical receipts, and your saved feedback instead of relying on one headline number.</p>
            </button>
            <Link href="/how-it-works" className="mt-3 inline-flex items-center gap-2 rounded-2xl border border-signal-blue/25 bg-signal-blue/10 px-3 py-2 text-xs font-black text-signal-blue transition hover:border-signal-blue/50 hover:bg-signal-blue/15">Why trust this? See how everything works</Link>
            {!isPro && <PremiumUpgradeCard headline="Unlock the full Top Signal breakdown" proof={`Pro users: ${proWinRateProof}% win rate across the last 30 tracked receipts`} detail="Free shows the signal and core reason. Pro opens deeper receipt history, premium analytics, and alert intelligence without changing the dashboard flow." onUpgrade={upgradeToPro} busy={upgrading || checkoutSyncing} />}
            <div className="mt-3 rounded-2xl border border-signal-blue/20 bg-black/20 p-3">
              <div className="mb-2 flex items-center justify-between gap-2">
                <p className="text-xs font-black uppercase tracking-[.16em] text-signal-blue">Why this signal?</p>
                <span className="rounded-full border border-white/10 bg-white/[.05] px-2 py-1 text-[11px] font-bold text-slate-300">Reason stack</span>
              </div>
              <ul className="space-y-1.5 text-sm text-slate-300">
                {signalReasonStack(top, snapshot.marketCondition, strategyDefinition.name).map(reason => <li key={reason} className="leading-5">• {reason}</li>)}
              </ul>
              <p className="mt-2 text-xs text-slate-500">Educational signal ranking based on live market data. Not financial advice.</p>
            </div>
          </div>

          <div className={`space-y-3 rounded-3xl border border-white/10 bg-gradient-to-br from-white/[.075] to-white/[.025] p-4 shadow-soft ${signalChanged ? 'animate-pulseSignal' : ''}`}>
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              ...
              <p className="mt-2 text-sm text-slate-300">{strategyDefinition.name} lens · {topStrategyFit.reason}</p>
              <p className="mt-2 text-xs font-bold text-slate-500">{signalDepth.confidenceExplanation}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <Breakdown signal={top} compact onGlossary={openGlossaryTerm} />
            </div>
            <div className="relative overflow-hidden rounded-2xl border border-signal-blue/20 bg-signal-blue/10 p-4">
              <div className={!isPro ? 'premium-lock blur-[1.5px] opacity-75' : ''}>
              <div className="mb-3 flex items-center justify-between gap-2">
                <p className="text-xs font-black uppercase tracking-[.16em] text-signal-blue">Signal depth</p>
                <span className="rounded-full border border-white/10 bg-black/20 px-2 py-1 text-[11px] font-bold text-slate-300">v23</span>
              </div>
              <p className="text-sm font-bold text-white">{signalDepth.headline}</p>
              <div className="mt-3 space-y-2">
                {signalDepth.factors.map(factor => <div key={factor.label} className="rounded-xl border border-white/10 bg-black/20 p-2"><div className="flex items-center justify-between gap-3"><p className="text-xs font-black text-slate-200">{factor.label}</p><p className="text-xs font-black text-signal-blue">{factor.value}%</p></div><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-signal-blue" style={{ width: factor.value + '%' }} /></div><p className="mt-1 text-[11px] leading-4 text-slate-400">{factor.detail}</p></div>)}
              </div>
              <div className="mt-3 rounded-xl border border-signal-amber/25 bg-signal-amber/10 p-3">
                <p className="text-xs font-black uppercase tracking-[.14em] text-signal-amber">Invalidation</p>
                <p className="mt-1 text-xs font-bold text-slate-200">{signalDepth.invalidation}</p>
                <p className="mt-1 text-xs text-slate-400">{signalDepth.actionCheckpoint}</p>
              </div>
              </div>
              {!isPro && <div className="absolute inset-x-3 bottom-3 rounded-2xl border border-signal-amber/25 bg-midnight-950/85 p-3 text-center backdrop-blur"><p className="text-xs font-black text-white"><Lock className="mr-1 inline text-signal-amber" size={13} />Full reason stack is Pro</p><button onClick={upgradeToPro} disabled={upgrading || checkoutSyncing} className="mt-2 rounded-xl border border-signal-amber/30 bg-signal-amber/10 px-3 py-1.5 text-xs font-black text-signal-amber disabled:opacity-60">Unlock signal depth</button></div>}
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
              <div className="flex h-full flex-col justify-between gap-2">
                <div><p className="text-xs font-black uppercase tracking-[.16em] text-slate-500">Confidence guide</p><p className={`mt-1 text-sm font-black ${confidenceBand(top.confidence).tone}`}>{confidenceBand(top.confidence).label}</p></div>
                <p className="text-xs text-slate-400">50–59 Watch · 60–71 Monitor · 72+ Act. {confidenceBand(top.confidence).copy}</p>
                <p className="text-xs font-bold text-slate-500">Session continuity: {sessionContinuityCopy}</p>
              </div>
            </div>
          ...
          <aside className="rounded-3xl border border-signal-amber/20 bg-signal-amber/10 p-4">
        ...
        <div className="mt-4 border-t border-white/10 pt-4">
          <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[.18em] text-slate-500">Secondary context</p>
              <h2 className="text-xl font-black text-white">Scan the rest after the decision loop.</h2>
            </div>
            <p className="max-w-xl text-sm text-slate-400">Watchlist and review rhythm now sit after Signal → Reason → Outcome, so mobile and desktop both keep the main path clean.</p>
          </div>
          <div className="grid gap-3 lg:grid-cols-2">
            <div className="rounded-3xl border border-white/10 bg-white/[.035] p-4">
              ...
              <p className="mt-3 text-xs text-slate-500">Secondary scan after the main read. The guided path still follows {top.symbol}.</p>
            </div>
        ...
        <div className="mt-4 border-t border-white/10 pt-4">
          <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[.18em] text-slate-500">Locked depth</p>
              <h2 className="text-xl font-black text-white">Learn from every signal after the main read.</h2>
            </div>
            <p className="max-w-xl text-sm text-slate-400">These panels support the guided flow without interrupting it: performance learning, Pro analytics, and receipts sit together below the decision path.</p>
          </div>
          <div className="grid gap-4 xl:grid-cols-3 xl:items-start">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[.035] p-4">
              <div className="mb-3 flex items-center gap-2 text-signal-blue"><History size={17} /><p className="text-xs font-black uppercase tracking-[.18em]">Recent signal outcomes</p></div>
              <div className="space-y-2">
                {recentSignalOutcomes.map(result => <SignalReceiptCard key={result.id} result={result} compact />)}
              </div>
              {plan !== 'pro' && <button onClick={upgradeToPro} className="mt-3 w-full rounded-2xl border border-signal-amber/30 bg-signal-amber/10 px-4 py-3 text-sm font-bold text-signal-amber"><Lock className="mr-2 inline" size={15} /> Unlock your edge</button>}
            </div>
          </div>
        </div>
      </section>

      <section className="mb-24 card rounded-3xl p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div><h2 className="text-xl font-black">Top Signal Grid</h2><p className="text-sm text-slate-400">Secondary scan after the main read. Showing 3 by default keeps the decision path focused.</p></div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setTopGridExpanded(value => !value)} className="rounded-2xl border border-white/10 bg-white/[.05] px-4 py-2 text-sm font-bold text-white">{topGridExpanded ? 'Show less' : 'Show more'}</button>
            <button onClick={() => openGlossaryTerm('Signal')} className="rounded-2xl border border-signal-blue/30 bg-signal-blue/10 px-4 py-2 text-sm font-bold text-signal-blue"><BookOpen className="mr-2 inline" size={16} /> Glossary</button>
          </div>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {visibleTopGridSignals.map(signal => <AssetCard key={signal.symbol} signal={signal} currency={currency} active={signal.symbol === active.symbol} starred={watchlist.includes(signal.symbol)} onSelect={() => selectSignal(signal.symbol)} onStar={() => toggleWatch(signal.symbol)} />)}
          {!isPro && lockedSignalCount > 0 && <LockedSignalPreview count={lockedSignalCount} onUpgrade={upgradeToPro} busy={upgrading || checkoutSyncing} />}
        </div>
      </section>

      <section className={`fixed bottom-3 z-50 overflow-hidden rounded-[1.75rem] border border-white/10 bg-midnight-950/95 shadow-soft backdrop-blur-xl transition-all ${controlsOpen ? 'inset-x-3 mx-auto max-h-[82vh] max-w-[1440px]' : 'left-3 max-h-[66px] w-[min(calc(100vw-1.5rem),360px)]'}`}>
        <button type="button" onClick={() => setControlsOpen(!controlsOpen)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left">
          <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-signal-blue/30 bg-signal-blue/10 text-signal-blue"><Settings2 size={18} /></span><div className="min-w-0"><p className="truncate text-sm font-black text-white">Settings + controls</p><p className="truncate text-xs text-slate-400">Account, mode, and notifications stay tucked away from the signal flow.</p></div></div>
          <ChevronDown className={`shrink-0 text-slate-400 transition ${controlsOpen ? 'rotate-180' : ''}`} size={20} />
        </button>
        <div className="max-h-[calc(82vh-74px)] overflow-y-auto border-t border-white/10 p-4">
      ...
      <footer className="pb-24 pt-8 text-center text-xs text-slate-500">Midnight Signal v{BUILD.version}  /  Monetization Layer  /  {performanceSource === 'database' ? 'Persistent signal results' : 'Simulated performance fallback'}  /  {snapshot.source}  /  Educational use only  /  Not financial advice</footer>
    </main>
  );
}
...
function ConfidenceLayerCard({ conviction, signal, strategyName, onGlossary }: { conviction: ConvictionBreakdown; signal: AssetSignal; strategyName: string; onGlossary: (term: string) => void }) {
  ...
  return <section className="mt-4 rounded-[2rem] border border-white/10 bg-gradient-to-br from-signal-blue/10 via-white/[.04] to-black/20 p-5 shadow-soft">
    <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
      <div>
        <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-signal-blue/30 bg-signal-blue/10 px-3 py-1 text-xs font-black uppercase tracking-[.16em] text-signal-blue"><ShieldCheck size={14} /> v18 Monetization Layer</div>
      ...
      {orbits.map((orbit) => (
        <div
          ...
          style={{
            ...
            ['--start-rotation' as string]: `${orbit.angle}deg`
          }}
        >
          <span
            className={`absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full ${orbit.dot}`}
            style={{ width: orbit.dotSize, height: orbit.dotSize }}
          />
        </div>
      ))}
        ...
        <div className="rounded-[1.65rem] border border-white/[.07] bg-white/[.025] p-4 text-center shadow-[inset_0_1px_0_rgba(255,255,255,.04)] lg:text-left">
          <div className="grid items-center gap-4 sm:grid-cols-[240px_1fr] lg:grid-cols-1">
            ...
            <div>
              ...
              <p className="mx-auto mt-2 max-w-lg text-sm font-semibold text-signal-blue lg:mx-0">We read the charts. You decide what to do. The system keeps score over time.</p>
          ...
          <div className="space-y-0">
            {ladder.map((item, index) => (
              <div key={item.title}>
                ...
                {index < ladder.length - 1 && <FlowArrow tone={ladder[index + 1].tone} />}
              </div>
            ))}
          </div>
        </div>
...
function GlossaryTerm({ term, children, onClick }: { term: string; children: React.ReactNode; onClick: (term: string) => void }) {
  return <button type="button" onClick={() => onClick(term)} className="font-bold text-signal-blue underline decoration-dotted decoration-signal-blue/70 underline-offset-4 transition hover:text-indigo-300 hover:decoration-indigo-300 active:opacity-70">{children}</button>;
}
...
function receiptMicroInsight(result: SignalResult) {
  if (result.outcome === 'win') return 'Signal matched expectation.';
  if (result.outcome === 'loss') return 'Missed continuation move.';
  return 'Mixed outcome; useful for calibration.';
}

function SignalReceiptCard({ result, compact = false }: { result: SignalResult; compact?: boolean }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[.03] p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-black">{result.symbol} <span className="text-xs font-normal text-slate-400">{result.direction.toUpperCase()}</span></p><p className="text-xs text-slate-400">{new Date(result.closedAt).toLocaleDateString()}  /  {formatHoldTime(result.openedAt, result.closedAt)}</p></div><span className={'rounded-full border px-2 py-1 text-[11px] font-black ' + performanceOutcomeClass(result.outcome)}>{outcomeLabel(result.outcome)}</span></div><div className="mt-3 rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-slate-400">Entry &rarr; Exit</p><p className="font-bold">{result.entryPrice} <span aria-hidden="true">&rarr;</span> {result.exitPrice}</p><p className={result.returnPct >= 0 ? 'text-sm font-black text-signal-green' : 'text-sm font-black text-signal-red'}>{result.returnPct >= 0 ? '+' : ''}{result.returnPct}%</p></div><p className="mt-2 text-xs font-bold text-slate-400">{receiptMicroInsight(result)}</p>{!compact && <><p className="mt-1 text-xs text-slate-300">{buildSignalReceiptText(result)}</p><p className="mt-1 text-xs text-slate-500">{result.note}</p></>}</div>;
}
...
function PremiumUpgradeCard({ headline, proof, detail, onUpgrade, busy }: { headline: string; proof: string; detail: string; onUpgrade: () => void; busy?: boolean }) {
  return <div className="mt-3 rounded-2xl border border-signal-amber/30 bg-gradient-to-br from-signal-amber/15 via-white/[.035] to-black/20 p-3">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0 flex-1">
        <p className="text-xs font-black uppercase tracking-[.16em] text-signal-amber"><Lock className="mr-1 inline" size={13} /> Pro edge</p>
        <p className="mt-1 text-sm font-black text-white">{headline}</p>
        <p className="mt-1 text-xs font-bold text-signal-amber">{proof}</p>
        <p className="mt-1 text-xs text-slate-400">{detail}</p>
      </div>
      <button onClick={onUpgrade} disabled={busy} className="rounded-2xl border border-signal-amber/30 bg-signal-amber/10 px-4 py-2 text-xs font-black text-signal-amber transition hover:bg-signal-amber/20 disabled:cursor-wait disabled:opacity-60">{busy ? 'Opening...' : 'Upgrade'}</button>
    </div>
  </div>;
}

function LockedSignalPreview({ count, onUpgrade, busy }: { count: number; onUpgrade: () => void; busy?: boolean }) {
  return <div className="relative min-h-[190px] overflow-hidden rounded-3xl border border-signal-amber/25 bg-white/[.035] p-4">
    <div className="premium-lock space-y-3 opacity-55 blur-[1.5px]">
      <div className="flex items-center justify-between"><div><p className="text-2xl font-black text-white">PRO</p><p className="text-sm text-slate-400">Premium signal</p></div><p className="text-3xl font-black text-signal-blue">82%</p></div>
      <div className="h-2 rounded-full bg-white/10"><div className="h-2 w-4/5 rounded-full bg-signal-blue" /></div>
      <p className="text-sm font-bold text-slate-200">Full reasoning, receipt history, and alert path are locked.</p>
      <div className="grid grid-cols-3 gap-2"><span className="h-12 rounded-2xl bg-white/10" /><span className="h-12 rounded-2xl bg-white/10" /><span className="h-12 rounded-2xl bg-white/10" /></div>
    </div>
    <div className="absolute inset-0 flex items-center justify-center bg-midnight-950/60 p-4 text-center backdrop-blur-[1px]">
      <div>
        <Lock className="mx-auto mb-2 text-signal-amber" size={22} />
        <p className="font-black text-white">{count} more signals locked</p>
        <p className="mt-1 text-xs text-slate-300">Upgrade to unlock premium signal depth and the full grid.</p>
        <button onClick={onUpgrade} disabled={busy} className="mt-3 rounded-2xl border border-signal-amber/30 bg-signal-amber/10 px-4 py-2 text-xs font-black text-signal-amber disabled:opacity-60">{busy ? 'Opening...' : 'Unlock Pro'}</button>
      </div>
    </div>
  </div>;
}

function TrackRecordStrip({ snapshot }: { snapshot: TrackRecordSnapshot }) {
  return <div className="mb-4 grid gap-2 rounded-3xl border border-white/10 bg-black/15 p-3 sm:grid-cols-2 lg:grid-cols-5"><BriefCard label="Last 20 accuracy" value={snapshot.last20WinRate ? snapshot.last20WinRate + '%' : '-'} detail={`${snapshot.wins} wins / ${snapshot.losses} losses`} /><BriefCard label="Avg tracked move" value={`${snapshot.avgReturn >= 0 ? '+' : ''}${snapshot.avgReturn}%`} detail={`${snapshot.total} settled receipts`} /><BriefCard label="Best signal type" value={snapshot.bestType} detail={snapshot.bestTypeWinRate ? snapshot.bestTypeWinRate + '% win rate' : 'Needs receipts'} /><BriefCard label="Active signals" value={String(snapshot.activeCount)} detail={`${snapshot.pending} pending checks`} /><BriefCard label="Last updated" value={snapshot.updatedLabel} detail="Track record is live" /></div>;
}
...
function FeedbackLoopCard({ signal, stats, globalStats, message, onFeedback }: { signal: AssetSignal; stats: FeedbackStats; globalStats: FeedbackStats; message: string; onFeedback: (action: FeedbackAction, outcome?: FeedbackOutcome) => void }) {
  return <div className="rounded-3xl border border-signal-blue/20 bg-signal-blue/10 p-4"><div className="mb-3 flex flex-wrap items-start justify-between gap-3"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full border border-signal-blue/30 bg-signal-blue/10 px-3 py-1 text-xs font-black uppercase tracking-[.16em] text-signal-blue"><DatabaseZap size={14} /> Feedback loop</div><h3 className="text-xl font-black">Did this signal play out?</h3><p className="mt-1 text-sm text-slate-300">Mark Yes, No, or Too early so the track record learns without changing the layout.</p></div><div className="grid grid-cols-2 gap-2 text-center text-xs sm:min-w-[220px]"><div className="rounded-2xl border border-white/10 bg-white/[.05] p-3"><p className="text-slate-400">This signal</p><p className="text-2xl font-black">{stats.total}</p></div><div className="rounded-2xl border border-white/10 bg-white/[.05] p-3"><p className="text-slate-400">All feedback</p><p className="text-2xl font-black">{globalStats.total}</p></div></div></div><div className="grid gap-2 sm:grid-cols-4"><button onClick={() => onFeedback('acted', 'win')} className="rounded-2xl border border-signal-green/30 bg-signal-green/10 px-3 py-3 text-sm font-black text-signal-green hover:bg-signal-green/15"><ThumbsUp className="mr-2 inline" size={16} /> Yes</button><button onClick={() => onFeedback('acted', 'loss')} className="rounded-2xl border border-signal-red/30 bg-signal-red/10 px-3 py-3 text-sm font-black text-signal-red hover:bg-signal-red/15"><ThumbsDown className="mr-2 inline" size={16} /> No</button><button onClick={() => onFeedback('acted', 'neutral')} className="rounded-2xl border border-signal-amber/30 bg-signal-amber/10 px-3 py-3 text-sm font-black text-signal-amber hover:bg-signal-amber/15">Too early</button><button onClick={() => onFeedback('ignored', null)} className="rounded-2xl border border-white/10 bg-white/[.05] px-3 py-3 text-sm font-black text-slate-200 hover:bg-white/10">Skipped</button></div><div className="mt-3 grid gap-2 sm:grid-cols-4"><BriefCard label="Acted" value={String(globalStats.acted)} detail="Total action taps" /><BriefCard label="Ignored" value={String(globalStats.ignored)} detail="Skipped signals" /><BriefCard label="Wins" value={String(globalStats.wins)} detail="User-marked wins" /><BriefCard label="User win rate" value={globalStats.winRate ? globalStats.winRate + '%' : '-'} detail="Wins / decisive" /></div>{message && <p className="mt-3 rounded-2xl border border-signal-green/20 bg-signal-green/10 p-3 text-sm font-bold text-signal-green">{message}</p>}</div>;
}
