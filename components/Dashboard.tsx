...
export default function Dashboard() {
  ...
  async function refreshPlan(): Promise<Plan> {
    ...
    try {
      ...
      setPlan(nextPlan);
      if (nextPlan === 'pro') setAuthMessage('Pro active â deeper signal intelligence is unlocked.');
      return nextPlan;
    } catch {
      setPlan('free');
      return 'free';
  ...
  useEffect(() => {
    ...
    if (params.get('checkout') === 'success') {
      ...
      setAuthMessage('Finalizing your Pro accessâ¦');
      ...
      const syncPlan = async () => {
        ...
        if (nextPlan === 'pro') {
          ...
          setAuthMessage('Pro active â deeper signal intelligence is unlocked.');
          if (interval) clearInterval(interval);
          return;
        }
        if (attempts >= 10) {
          setCheckoutSyncing(false);
          setAuthMessage('Payment received. Pro is still syncing â refresh once or try Sync Plan in a few seconds.');
          if (interval) clearInterval(interval);
        }
      };
  ...
  async function recordSignalFeedback(signal: AssetSignal, action: FeedbackAction, outcome: FeedbackOutcome = null) {
    ...
    setFeedback(next);
    writeFeedback(next);
    setFeedbackMessage(action === 'ignored' ? signal.symbol + ' ignored â learning loop updated.' : signal.symbol + ' marked as ' + (outcome || 'acted') + '.');
    if (authUser?.id) {
    ...
    return (
      <main className="mx-auto min-h-screen max-w-6xl px-4 py-5 sm:px-6 lg:px-8" suppressHydrationWarning>
        <section className="card rounded-3xl p-6">
          ...
          <h1 className="mt-2 text-3xl font-black">Loading signal dashboardâ¦</h1>
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-5 sm:px-6 lg:px-8">
      ...
      <header className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-signal-blue/20 bg-signal-blue/10 px-3 py-1 text-xs font-semibold text-signal-blue"><Sparkles size={14} /> v{BUILD.version} Â· Confidence Layer</div>
          <h1 className="text-4xl font-black tracking-tight sm:text-5xl">Whatâs the signal tonight? <span className="text-signal-blue">ð</span></h1>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.35fr_.85fr]">
        <div className="card rounded-3xl p-5">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <BriefCard label="Win rate" value={`${performanceSummary.winRate}%`} detail={`${performanceSummary.wins} wins Â· ${performanceSummary.losses} losses from decisive outcomes`} />
      ...
      <footer className="py-8 text-center text-xs text-slate-500">Midnight Signal v{BUILD.version} Â· Confidence Layer Â· {performanceSource === 'database' ? 'Persistent signal results' : 'Simulated performance fallback'} Â· {snapshot.source} Â· Educational use only Â· Not financial advice</footer>
    </main>
  );
}
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[.04] p-5">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 text-center">
          <div className="rounded-2xl border border-white/10 bg-black/20 p-3"><p className="text-xs text-slate-500">Win rate</p><p className="text-xl font-black">{performance.winRate || 'â'}{performance.winRate ? '%' : ''}</p></div>
  ...
  return <section className="mb-4 rounded-[2rem] border border-signal-blue/20 bg-gradient-to-br from-signal-blue/10 via-white/[.03] to-signal-amber/10 p-5 shadow-soft">
    <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <h2 className="text-2xl font-black">BTC Â· ADA Â· NIGHT basket intelligence</h2>
      ...
      <div className="rounded-3xl border border-white/10 bg-black/20 px-5 py-4 text-right">
        ...
        <p className="text-4xl font-black text-white">{insight.score || 'â'}</p>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-3">
      ...
      <BriefCard label="Top contributor" value={insight.strongest?.symbol || 'â'} detail={insight.topContributor} />
  ...
  return (
    <section className="rounded-[2rem] border border-signal-blue/20 bg-signal-blue/10 p-5 shadow-soft">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <BriefCard label="Preferred assets" value={profile.preferredAssets[0] || 'None yet'} detail={profile.preferredAssets.slice(1, 3).join(' Â· ') || 'Built from behavior'} />
      ...
      {lead && <div className="mt-4 rounded-3xl border border-white/10 bg-black/15 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 text-2xl font-black text-white">{lead.symbol} Â· {lead.personalScore}% fit</h3>
...
function SignalDiscoveryHero({ userTop, globalTop, gap, shouldExpose, isPro, currency, userSummary, globalSummary, globalInWatchlist, conversionCount, onSelect, onAddGlobal, onTrackGlobal, onUpgrade }: { userTop: AssetSignal; globalTop: AssetSignal; gap: number; shouldExpose: boolean; isPro: boolean; currency: string; userSummary: ReturnType<typeof summarizePerformance>; globalSummary: ReturnType<typeof summarizePerformance>; globalInWatchlist: boolean; conversionCount: number; onSelect: (symbol: string) => void; onAddGlobal: () => void; onTrackGlobal: () => void; onUpgrade: () => void }) {
  ...
  return <section className="mb-4 grid gap-4 lg:grid-cols-2">
    ...
    <div className={(locked ? 'premium-lock ' : '') + "rounded-[2rem] border border-signal-amber/30 bg-gradient-to-br from-signal-amber/15 via-white/[.04] to-signal-blue/10 p-5 sm:p-6"}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-5xl font-black sm:text-6xl">{globalTop.symbol}</h2><p className="mt-1 text-slate-400">{globalTop.name} Â· outside your watchlist universe unless added</p></div><span className={'rounded-full border px-4 py-2 text-sm font-bold ' + labelClass(globalTop.label)}>{globalTop.label}</span></div>
...
function PatternBrief({ pattern, fallback }: { pattern?: UserIntelligenceProfile['patterns'][number]; fallback: string }) {
  ...
  return <div className={'rounded-3xl border p-4 ' + tone}><p className="text-xs font-black uppercase tracking-[.16em]">{pattern?.direction || 'learning'}</p><p className="mt-2 text-lg font-black text-white">{pattern?.title || fallback}</p><p className="mt-1 text-sm text-slate-300">{pattern?.description || 'Keep using feedback controls and signal outcomes to train this insight.'}</p>{pattern && <p className="mt-2 text-xs font-bold">{pattern.confidence}% confidence Â· {pattern.action}</p>}</div>;
}
...
function SignalRow({ signal, currency, active, starred, onSelect, onStar }: { signal: AssetSignal; currency: string; active: boolean; starred: boolean; onSelect: () => void; onStar: () => void }) { return <div className={`flex items-center justify-between rounded-2xl border p-3 ${active ? 'border-signal-blue/50 bg-signal-blue/10' : 'border-white/10 bg-white/[.04]'}`}><button onClick={onSelect} className="text-left"><p className="font-bold">{signal.symbol} <span className="text-sm font-normal text-slate-400">{signal.name}</span></p><p className="text-sm text-slate-300">{formatPrice(signal.price, currency)} Â· {signal.confidence}%</p></button><button onClick={onStar} className="rounded-xl p-2 text-signal-amber hover:bg-white/10"><Star fill={starred ? 'currentColor' : 'none'} /></button></div>; }
...
function ResultMini({ result, title }: { result: SignalResult; title?: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[.03] p-3"><div className="flex items-center justify-between gap-2"><p className="font-black">{title ? title + ': ' : ''}{result.symbol}</p><span className={'rounded-full border px-2 py-1 text-[11px] font-black ' + performanceOutcomeClass(result.outcome)}>{outcomeLabel(result.outcome)}</span></div><p className={result.returnPct >= 0 ? 'mt-2 text-lg font-black text-signal-green' : 'mt-2 text-lg font-black text-signal-red'}>{result.returnPct >= 0 ? '+' : ''}{result.returnPct}%</p><p className="text-xs text-slate-400">{result.direction.toUpperCase()} Â· {result.confidence}% confidence Â· {formatHoldTime(result.openedAt, result.closedAt)}</p></div>;
}

function SignalReceiptCard({ result, compact = false }: { result: SignalResult; compact?: boolean }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[.03] p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-black">{result.symbol} <span className="text-xs font-normal text-slate-400">{result.direction.toUpperCase()}</span></p><p className="text-xs text-slate-400">{new Date(result.closedAt).toLocaleDateString()} Â· {formatHoldTime(result.openedAt, result.closedAt)}</p></div><span className={'rounded-full border px-2 py-1 text-[11px] font-black ' + performanceOutcomeClass(result.outcome)}>{outcomeLabel(result.outcome)}</span></div><div className="mt-3 rounded-xl border border-white/10 bg-black/10 p-3"><p className="text-xs text-slate-400">Entry â Exit</p><p className="font-bold">{result.entryPrice} â {result.exitPrice}</p><p className={result.returnPct >= 0 ? 'text-sm font-black text-signal-green' : 'text-sm font-black text-signal-red'}>{result.returnPct >= 0 ? '+' : ''}{result.returnPct}%</p></div>{!compact && <><p className="mt-2 text-xs text-slate-300">{buildSignalReceiptText(result)}</p><p className="mt-1 text-xs text-slate-500">{result.note}</p></>}</div>;
}

function SignalPerformanceCard({ signal, outcome }: { signal: AssetSignal; outcome: SignalOutcome }) {
  return <div className="rounded-3xl border border-white/10 bg-white/[.04] p-4"><div className="mb-3 flex items-center gap-2 text-signal-blue"><Activity size={18} /><h3 className="font-bold">Previous Signal Result</h3></div><span className={'inline-flex rounded-full border px-3 py-1 text-xs font-black ' + outcomeClass(outcome)}>{outcome}</span><p className="mt-3 text-sm text-slate-300">{signal.symbol} is reviewed through a simple 24h outcome lens: Worked / Failed / Neutral. This keeps the product honest without pretending to be a guaranteed trading system.</p><p className="mt-3 text-xs text-slate-500">Educational heuristic only Â· not financial advice</p></div>;
}
...
function AnalyticsBreakdownTable({ title, rows }: { title: string; rows: ReturnType<typeof buildProPerformanceAnalytics>['bySymbol'] }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[.03] p-3"><p className="mb-2 text-xs font-bold uppercase tracking-[.16em] text-slate-400">{title}</p><div className="space-y-2">{rows.map(row => <div key={row.key} className="grid grid-cols-[1fr_auto_auto] items-center gap-2 rounded-xl border border-white/10 bg-black/10 px-3 py-2 text-xs"><span className="font-bold text-slate-200">{row.label}</span><span className="text-slate-400">{row.total}x</span><span className={row.avgReturn >= 0 ? 'font-black text-signal-green' : 'font-black text-signal-red'}>{row.winRate}% Â· {row.avgReturn >= 0 ? '+' : ''}{row.avgReturn}%</span></div>)}</div></div>;
}
...
function JourneyCard({ visits, journey, completed }: { visits: number; journey: { title: string; stage: number; progress: number; note: string }; completed: number }) { return <div className="card rounded-3xl p-5"><div className="mb-3 flex items-center gap-2 text-signal-blue"><Trophy size={18} /><h2 className="text-xl font-bold">Your Signal Journey</h2></div><p className="text-2xl font-black">{journey.title}</p><p className="mt-1 text-sm text-slate-300">Stage {journey.stage} Â· {visits} visits Â· {completed}/4 ritual steps</p><div className="mt-4 h-3 rounded-full bg-white/10"><div className="h-3 rounded-full bg-signal-blue" style={{ width: `${journey.progress}%` }} /></div><p className="mt-3 text-sm text-slate-300">{journey.note}</p></div>; }
function RitualCard({ ritual, mark }: { ritual: RitualState; mark: (key: keyof RitualState) => void }) { const items: [keyof RitualState, string][] = [['topSignal', 'Check Top Signal'], ['confidence', 'Review Confidence'], ['watchlist', 'Scan Watchlist'], ['market', 'Read Market Condition']]; return <div className="card rounded-3xl p-5"><div className="mb-3 flex items-center gap-2 text-signal-blue"><CheckCircle2 size={18} /><h2 className="text-xl font-bold">Tonightâs Ritual</h2></div><div className="space-y-2">{items.map(([key, label]) => <button key={key} onClick={() => mark(key)} className={`flex w-full items-center justify-between rounded-2xl border p-3 text-left ${ritual[key] ? 'border-signal-green/30 bg-signal-green/10 text-signal-green' : 'border-white/10 bg-white/[.04] text-slate-200'}`}><span>{label}</span><CheckCircle2 size={18} /></button>)}</div></div>; }
