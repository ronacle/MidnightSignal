  ...
  return (
    <section className="card trust-dashboard-panel" id="trust-dashboard">
      <div className="row space-between">
        <div>
          <div className="eyebrow">Outcome tracking</div>
      ...
      <div className="factor-grid trust-stat-grid">
        <div className="factor-chip"><span>Recent hit rate</span><strong>{dashboard.hitRate ?? '—'}{dashboard.hitRate !== null ? '%' : ''}</strong></div>
        <div className="factor-chip"><span>Avg follow-through</span><strong>{dashboard.avgFollowThrough ?? '—'}{dashboard.avgFollowThrough !== null ? '%' : ''}</strong></div>
        <div className="factor-chip"><span>Worked / Mixed / Failed</span><strong>{dashboard.workedCount} / {dashboard.mixedCount} / {dashboard.failedCount}</strong></div>
          ...
          <div className="history-stack">
            {dashboard.recentOutcomes.length ? dashboard.recentOutcomes.map((entry) => (
              <div className="history-row trust-history-row" key={entry.id}>
                ...
                <span>{entry.latestHorizon === 'developing' ? 'Still forming' : `${entry.latestHorizon} ${entry.latestReturnPct}%`}</span>
