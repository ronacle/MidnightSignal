export function buildProfileSnapshot(state = {}) {
  return {
    ...
    restoreLastSelectedAsset: state.restoreLastSelectedAsset !== false,
    restorePanelState: state.restorePanelState !== false,
    preferredDashboardFocus: state.preferredDashboardFocus || 'top-signal',
    soundProfile: state.soundProfile || 'beacon-soft',
...
export function applyProfileSnapshot(state = {}, profile = {}) {
  return {
    ...
    restoreLastSelectedAsset: typeof profile.restoreLastSelectedAsset === 'boolean' ? profile.restoreLastSelectedAsset : state.restoreLastSelectedAsset,
    restorePanelState: typeof profile.restorePanelState === 'boolean' ? profile.restorePanelState : state.restorePanelState,
    preferredDashboardFocus: profile.preferredDashboardFocus || state.preferredDashboardFocus || 'top-signal',
    soundProfile: profile.soundProfile || state.soundProfile || 'beacon-soft',
  };
}
