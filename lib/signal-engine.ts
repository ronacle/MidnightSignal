...
function formatAssetName(raw: RawCoin) {
  return raw.symbol.toUpperCase();
}
...
export async function generateDashboard(): Promise<DashboardPayload> {
  try {
    ...
    const grid = markets.map(scoreFromMarket);
    return buildPayload(grid, "coingecko");
  } catch {
