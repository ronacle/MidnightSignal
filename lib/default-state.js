export const DEFAULT_STATE = {
  mode: 'Beginner',
  currency: 'USD',
  strategy: 'Swing',
  timeframe: '1H',
  selectedAsset: 'BTC',
  profileId: null,
  profileName: 'My setup',
  watchlist: ['BTC', 'ETH', 'ADA', 'SOL'],
  savedProfiles: [null, null, null],
  alerts: [],
  acceptedDisclaimer: false,
  lastViewedAt: null,
  updatedAt: null,
  deviceLabel: 'This device',
  liveUpdatesEnabled: true,
  liveRefreshInterval: '60',
  livePulseEnabled: true,
  signalSoundsEnabled: false,
  alertDeliveryEnabled: false,
  alertDeliveryEmail: '',
  alertCooldownMinutes: '30',
  alertDigestMode: 'instant',
  alertDigestIntervalMinutes: '240',
  alertLastDeliveryAt: null,
  alertLastDeliveryStatus: '',
  alertCenterThreshold: '70',
  alertCenterDirection: 'both',
  alertCenterTimeframe: '15M',
  alertCenterRuleType: 'conviction_above',
  alertCenterImportance: 'important',
  recentAlertEvents: [],
  signalStreamPreferences: {
    enabledTypes: ['lead_shift', 'conviction_change', 'decision_update', 'trigger_hit', 'market_tone', 'watchlist_move', 'custom_rule', 'signal_event'],
    minimumImportance: 'all',
    watchlistOnly: false,
    quietMode: false,
  },
  alertMemory: { assetMap: {}, triggerLog: {} },
  alertDigestMemory: { queued: [], lastSentAt: null },
  entitlement: {
    status: 'inactive',
    verified: false,
    source: 'local',
    customerId: null,
    subscriptionId: null,
    checkoutSessionId: null,
    priceId: null,
    currentPeriodEnd: null,
    checkedAt: null,
    lastEventAt: null,
    lastError: ''
  },
  stripeWebhookStatus: '',
  stripeWebhookUpdatedAt: null,
  planTier: 'basic',
  dashboardFocus: 'signals',
  onboardingGoal: '',
  onboardingCompletedAt: null,
  userType: 'Beginner',
  intent: 'learn',
  modeEngineVersion: '11.93',
  personalSignalMemoryVersion: '11.93'
};

export const MARKET_FIXTURES = [
  { symbol: 'BTC', name: 'Bitcoin', conviction: 78, sentiment: 'bullish', story: 'Holding trend structure while momentum remains constructive.' },
  { symbol: 'ETH', name: 'Ethereum', conviction: 63, sentiment: 'neutral', story: 'Strength improving, but still waiting on cleaner follow-through.' },
  { symbol: 'ADA', name: 'Cardano', conviction: 74, sentiment: 'bullish', story: 'Leadership tone improving with a stronger recovery posture.' },
  { symbol: 'SOL', name: 'Solana', conviction: 46, sentiment: 'neutral', story: 'Momentum is mixed after a quick stretch higher.' },
  { symbol: 'XRP', name: 'XRP', conviction: 39, sentiment: 'bearish', story: 'Still lagging relative strength leaders.' }
];
