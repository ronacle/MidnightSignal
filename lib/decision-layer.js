...
function buildDecisionFeedback(asset, posture, convictionTag) {
  const score = Number(asset?.signalScore ?? asset?.conviction ?? 50);
  const sentiment = String(asset?.sentiment || 'neutral').toLowerCase();
  const regime = String(asset?.marketRegime || 'Mixed');

  let action = 'WAIT';
  let tone = 'wait';
  let rationale = 'Let the next move sharpen the read before committing harder.';

  if (score < 42) {
    action = 'AVOID';
    tone = 'avoid';
    rationale = 'The edge is too thin right now, so patience is the cleaner read.';
  } else if ((posture || '').toLowerCase().includes('defensive') || ((posture || '').toLowerCase().includes('bearish') && score >= 60)) {
    action = 'REDUCE';
    tone = 'reduce';
    rationale = 'Pressure is unresolved, so risk control matters more than forcing the setup.';
  } else if (sentiment === 'bullish' && score >= 74 && regime !== 'Chop') {
    action = 'LEAN IN';
    tone = 'lean';
    rationale = 'Strength is present, but only while confirmation stays intact.';
  } else if (sentiment === 'bullish' && score >= 58) {
    action = 'WAIT';
    tone = 'building';
    rationale = 'The setup is improving, but it still needs confirmation before leaning harder.';
  }

  const confidence = score >= 72
    ? 'Higher-confidence decision read'
    : score >= 55
      ? 'Moderate-confidence decision read'
      : 'Cautious decision read';

  return { action, tone, rationale, confidence, confidenceLabel: confidence };
}
...
export function buildDecisionLayer(asset, previousEntry = null) {
  ...
  const decisionFeedback = buildDecisionFeedback(asset, posture, convictionTag);
  ...
  return {
    ...
    actionBias,
    decisionAction: decisionFeedback.action,
    decisionTone: decisionFeedback.tone,
    decisionRationale: decisionFeedback.rationale,
    decisionConfidence: decisionFeedback.confidence,
    supportingRead,
    riskContext,
    bestFor,
    cues,
