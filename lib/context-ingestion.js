...
function normalizedSourceType(value = '') {
  ...
  if (['nightly', 'brief', 'nightly-brief', 'nightly-note'].includes(lowered)) return 'brief';
  return 'note';
}
...
function inferTopicBucket(item = {}) {
  ...
  if (/nightly brief|signal watch|market watch|community pulse|ecosystem insight/.test(text)) return 'Nightly';
  return 'General';
}
...
export function normalizeContextItem(item = {}) {
  ...
  const sourceType = normalizedSourceType(item.sourceType || item.type || item.kind || item.format);
  const sourceNetwork = normalizedSourceNetwork(item);
  const sourceHandle = cleanText(item.sourceHandle || item.handle || item.author || item.username || '');
  const timestamp = normalizeTimestamp(item.timestamp || item.publishedAt || item.pubDate || item.createdAt);
  ...
  const externalId = cleanText(item.externalId || item.guid || item.postId || item.tweetId || item.entryId || item.idStr || '');
  const imageUrl = normalizeUrl(item.imageUrl || item.image || item.thumbnail || item.previewImage || '');
  ...
  return {
    ...
    meta: {
      ...
      hooks: {
        ...
        fields: ['headline/title', 'summary/body', 'url', 'assetMentions', 'sentimentHint', 'publishedAt', 'sourceNetwork', 'sourceHandle', 'topicBucket', 'externalId', 'imageUrl'],
      },
    },
  };
}
