...
function buildAgreement(timeframe = {}) {
  const values = [timeframe.tf5m, timeframe.tf15m, timeframe.tf1h].filter((value) => typeof value === 'number');
  if (!values.length) {
    return { score: 50, label: 'Mixed agreement', spread: 0 };
  }
  const spread = Math.max(...values) - Math.min(...values);
  const score = Math.round(clamp(100 - spread * 2.2, 32, 100));
  let label = 'Mixed agreement';
  if (score >= 82) label = 'Aligned';
  else if (score <= 52) label = 'Conflicted';
  return { score, label, spread };
}

function buildMomentumState(timeframe = {}) {
  const tf5 = Number(timeframe.tf5m || 0);
  const tf15 = Number(timeframe.tf15m || 0);
  const tf1h = Number(timeframe.tf1h || 0);

  if (tf5 > tf15 && tf15 >= tf1h) return 'Accelerating';
  if (tf5 < tf15 && tf15 <= tf1h) return 'Cooling';
  if (Math.abs(tf5 - tf15) <= 4 && Math.abs(tf15 - tf1h) <= 4) return 'Stable';
  return 'Mixed';
}

function buildVolatilityState(priceRange24h = 0, change24h = 0) {
  const range = Number(priceRange24h || 0);
  const move = Math.abs(Number(change24h || 0));
  if (range <= 3 && move <= 2) return 'Cooling';
  if (range >= 12 || move >= 8) return 'Expanding';
  if (range >= 7 || move >= 4) return 'Active';
  return 'Balanced';
}

function buildCredibility(asset, timeframe, factors) {
  const agreement = buildAgreement(timeframe);
  const liquidityBoost = Number(asset.volumeToMarketCap || 0) >= 6 ? 4 : 0;
  const structureBoost = Number(asset.distanceFromHigh24h || -10) >= -3.5 ? 4 : 0;
  const penalty = agreement.score <= 52 ? 6 : 0;
  const confidence = Math.round(clamp((factors.momentum * 0.35) + (agreement.score * 0.25) + (factors.relativeStrength * 0.2) + (factors.liquidity * 0.1) + (factors.structure * 0.1) + liquidityBoost + structureBoost - penalty, 26, 99));

  return {
    confidence,
    agreement: agreement.label,
    agreementScore: agreement.score,
    agreementSpread: agreement.spread,
    momentumState: buildMomentumState(timeframe),
    volatilityState: buildVolatilityState(asset.priceRange24h, asset.change24h),
  };
}
...
function buildWhyNow(asset, credibility) {
  return [
    `Trend ${credibility.agreement === 'Aligned' ? 'agrees' : credibility.agreement === 'Conflicted' ? 'conflicts' : 'is mixed'} across 5m, 15m, and 1h.`,
    `Momentum is ${credibility.momentumState.toLowerCase()} right now.`,
    `Volatility is ${credibility.volatilityState.toLowerCase()} instead of random.`
  ];
}

function buildWatchNext(asset, credibility) {
  if (credibility.agreement === 'Conflicted') return 'Wait for stronger timeframe agreement before trusting a breakout.';
  if (credibility.momentumState === 'Accelerating') return 'Watch for follow-through on the next refresh to confirm leadership.';
  if (credibility.momentumState === 'Cooling') return 'Watch whether short-term momentum stabilizes before chasing strength.';
  if (credibility.volatilityState === 'Expanding') return 'Watch for volatility to settle so the posture can hold.';
  return 'Watch for the next refresh to see if conviction stays steady.';
}

function buildPostureSummary(asset, credibility) {
  const tone = Number(asset.signalScore || 0) >= 68 ? 'Bullish' : Number(asset.signalScore || 0) <= 44 ? 'Bearish' : 'Balanced';
  if (credibility.agreement === 'Conflicted') return `${tone}, but timeframe conflict is lowering trust.`;
  if (credibility.momentumState === 'Accelerating') return `${tone}, with momentum improving into the latest refresh.`;
  if (credibility.momentumState === 'Cooling') return `${tone}, but short-term energy is fading.`;
  return `${tone}, with a steadier posture across the board.`;
}
...
export function scoreAsset(asset, marketContext = {}) {
  ...
  const credibility = buildCredibility(asset, timeframe, factors);
  ...
  const enriched = {
    ...
    weights,
    timeframe,
    confidenceScore: credibility.confidence,
    timeframeAgreement: credibility.agreement,
    timeframeAgreementScore: credibility.agreementScore,
    momentumState: credibility.momentumState,
    volatilityState: credibility.volatilityState,
    signalLabel: buildSignalLabel({ ...asset, signalScore }),
    signalReasons: buildReasons({ ...asset, signalScore }),
    signalDrivers: buildWhyNow({ ...asset, signalScore }, credibility),
    watchNext: buildWatchNext({ ...asset, signalScore }, credibility),
    postureSummary: buildPostureSummary({ ...asset, signalScore }, credibility),
    factors,
  };
