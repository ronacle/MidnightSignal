...
export function deriveExperienceProfile(state = {}) {
  ...
  if (intent === 'track') { profile.showContextPanel = true; profile.showSinceLastVisit = true; profile.boardSubtitle = "Tonight's strongest signals, surfaced."; profile.marketEyebrow = 'Focused board'; profile.heroChecklist = ['The board is the main workspace.','Since Last Visit stays visible for quick catch-up.','You can compare more names without changing screens.']; }
...
export function applyModePreset(state = {}, override = {}) {
  ...
  return { ...state, userType: base.userType, intent: base.intent, mode: base.recommended.mode, strategy: base.recommended.strategy, timeframe: base.recommended.timeframe, livePulseEnabled: base.recommended.livePulseEnabled, signalSoundsEnabled: base.recommended.signalSoundsEnabled, dashboardFocus: base.recommended.dashboardFocus, onboardingGoal: base.intent, onboardingCompletedAt: state.onboardingCompletedAt || new Date().toISOString(), modeEngineVersion: '11.83' };
}
