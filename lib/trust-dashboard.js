...
export function classifyOutcomeFromReturn(returnPct) {
  if (typeof returnPct !== 'number') {
    return { label: 'Still developing', tone: 'developing', summary: 'Not enough follow-through data yet.' };
  }
  if (returnPct >= 1.5) {
    return { label: 'Worked', tone: 'worked', summary: 'Positive follow-through after the signal.' };
  }
  if (returnPct <= -1.25) {
    return { label: 'Failed', tone: 'failed', summary: 'The move faded after the signal.' };
  }
  return { label: 'Mixed', tone: 'mixed', summary: 'The signal moved, but follow-through was limited.' };
}
...
function modeCopy(mode = 'Beginner') {
  if (mode === 'Active trader') {
    return {
      ...
      subtitle: 'Recent follow-through, hit rate, and where continuation has actually been showing up.',
      takeawayPrefix: 'Tactical read',
    };
  }
  if (mode === 'Long-term') {
    return {
      title: 'Trust dashboard',
      subtitle: 'Recent higher-timeframe follow-through so you can judge whether signals have been carrying.',
      takeawayPrefix: 'Trend read',
    };
  }
  return {
    title: 'Trust dashboard',
    subtitle: 'A simple look at how recent signals have followed through so Midnight Signal feels more understandable over time.',
    takeawayPrefix: 'Simple read',
  };
}
