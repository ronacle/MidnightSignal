...
function getDirectionalReturn(entry, returnPct) {
  ...
  if (direction === 'bearish') return round(-returnPct);
  return returnPct;
}

function classifyDecisionResult(entry, directionalReturn) {
  ...
  if (action === 'LEAN IN') {
    ...
    if (directionalReturn <= -1) return 'Missed';
    return 'Mixed';
  }
  if (action === 'REDUCE' || action === 'AVOID') {
    if (directionalReturn <= -1) return 'Protected';
    if (directionalReturn >= 1.25) return 'Missed upside';
    return 'Mixed';
  }
  if (action === 'WAIT') {
...
export function buildForwardOutcomeEntries(entries = []) {
  return (entries || []).map((entry) => {
    ...
    return {
      ...
      directionalReturnPct: getDirectionalReturn(entry, latest.returnPct),
      decisionAction: entry.decisionAction || 'WAIT',
      decisionTone: entry.decisionTone || 'wait',
      decisionConfidence: entry.decisionConfidence || null,
      decisionResult: classifyDecisionResult(entry, getDirectionalReturn(entry, latest.returnPct)),
    };
  });
}
...
function getReliabilityState(hitRate) {
  if (typeof hitRate !== 'number') return 'Building';
  if (hitRate >= 65) return 'Strong';
  if (hitRate >= 45) return 'Moderate';
  return 'Cautious';
}
...
export function buildTrustDashboard({ forwardValidation = [], recentAlertEvents = [], mode = 'Beginner' } = {}) {
  ...
  resolved.forEach((entry) => {
    const action = String(entry.decisionAction || 'WAIT').toUpperCase();
    decisionBuckets[action] = decisionBuckets[action] || { action, samples: 0, effective: 0, protected: 0, missed: 0, mixed: 0, returns: [] };
    const bucket = decisionBuckets[action];
    if (typeof entry.directionalReturnPct !== 'number') return;
    bucket.samples += 1;
    bucket.returns.push(entry.directionalReturnPct);
    const result = String(entry.decisionResult || 'Mixed').toLowerCase();
    if (result.includes('effective')) bucket.effective += 1;
    else if (result.includes('protected')) bucket.protected += 1;
    else if (result.includes('missed')) bucket.missed += 1;
    else bucket.mixed += 1;
  });
  ...
  })).sort((a, b) => b.samples - a.samples).slice(0, 4);

  const bestDecision = decisionPerformance
    .filter((item) => typeof item.effectivenessRate === 'number')
    .sort((a, b) => b.effectivenessRate - a.effectivenessRate)[0] || null;

  const decisionTakeaway = bestDecision
    ? `${bestDecision.action} reads have been ${bestDecision.effectivenessRate}% effective across ${bestDecision.samples} resolved sample${bestDecision.samples === 1 ? '' : 's'}.`
    : 'Decision feedback is building as signals collect follow-through.';
  ...
  const reliabilityState = getReliabilityState(hitRate);
  ...
  return {
    ...
    takeaway,
    reliabilityState,
    reliabilityLabel: hitRate !== null ? `${reliabilityState} recently` : 'Building reliability',
    decisionPerformance,
    bestDecision,
    decisionTakeaway,
  };
