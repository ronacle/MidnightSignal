...
function formatFreshness(timestamp) {
  ...
  if (!Number.isFinite(diffMs) || diffMs < 0) return 'recent';
...
function buildNightlyBrief(asset = {}, liveContext = {}, catalystFeed = []) {
  const items = Array.isArray(liveContext?.items) ? liveContext.items : [];
  const topCatalysts = (catalystFeed || []).slice(0, 3);
  const cardanoMidnightItems = items.filter((item) => ['Cardano', 'Midnight'].includes(item.topicBucket)).slice(0, 3);
  const leadOne = topCatalysts[0];
  const leadTwo = topCatalysts[1];
  const leadThree = topCatalysts[2];

  const developments = [leadOne, leadTwo, leadThree]
    .filter(Boolean)
    .map((item) => ({
      headline: item.title,
      detail: `${item.matchLabel || 'Context match'} · ${formatFreshness(item.timestamp)}`,
    }));

  while (developments.length < 3) {
    developments.push({
      headline: developments.length === 0 ? `${asset.symbol || 'Market'} context is still organizing` : 'Waiting for another confirmed catalyst',
      detail: developments.length === 0 ? 'Live narrative coverage is still warming up.' : 'The bridge will add more once fresh items arrive.',
    });
  }
  ...
  return {
    ...
    developments,
    signalWatch: leadOne
      ? `${asset.symbol || 'Lead asset'} signal watch: ${leadOne.title}`
      : `${asset.symbol || 'Lead asset'} signal watch: confirmation still matters more than narrative volume.`,
    marketWatch: leadTwo
      ? `${leadTwo.topicBucket || 'Market'} watch: ${leadTwo.title}`
      : 'Market watch: broader leadership is still setting the tone tonight.',
    communityPulse: cardanoMidnightItems[0]
      ? `${cardanoMidnightItems[0].title} · ${formatFreshness(cardanoMidnightItems[0].timestamp)}`
      : 'Community pulse: Cardano / Midnight coverage is light but the bridge is ready.',
    ecosystemFocus: cardanoMidnightItems.length
      ? cardanoMidnightItems.map((item) => ({
          headline: item.title,
          detail: `${item.topicBucket || 'Ecosystem'} · ${formatFreshness(item.timestamp)}`,
        }))
      : [{ headline: 'No Cardano / Midnight ecosystem catalyst yet', detail: 'The app will surface it here once relevant context is ingested.' }],
  };
}
...
function summarizeCatalystBody(item = {}) {
  const body = String(item.body || '').trim();
  if (!body) return 'Fresh context is available, but the catalyst summary is still forming.';
  if (body.length <= 140) return body;
  return `${body.slice(0, 137).trim()}...`;
}

function buildFallbackCatalystFeed(asset = {}, rankedAssets = [], regimeSummary = {}) {
  const leaders = (rankedAssets || []).slice(0, 3).map((item) => item.symbol).filter(Boolean);
  const regime = regimeSummary?.regime || asset.marketRegime || 'Mixed';
  const symbol = asset.symbol || 'Asset';
  ...
  return [
    {
      id: `${symbol}-market-tone`,
      catalystType: 'Market',
      title: `${leaders.join(' · ') || symbol} are shaping tonight's tape`,
      body: `Leadership is being set by ${leaders.join(', ') || symbol}, so ${symbol} is being judged against a ${String(regime).toLowerCase()} market backdrop.`,
      source: 'Signal engine',
      sourceType: 'note',
      assetMentions: [symbol, ...leaders].filter(Boolean),
      relevanceScore: 3.2,
      sentimentHint: asset.sentiment === 'bearish' ? 'cautious' : 'constructive',
      matchLabel: leaders.includes(symbol) ? 'Direct context match' : 'Market backdrop',
    },
    {
      id: `${symbol}-setup-driver`,
      catalystType: symbol === 'ADA' ? 'Cardano' : 'Ecosystem',
      title: `${symbol} posture is being driven by its internal setup`,
      body: `${symbol} is scoring as ${asset.signalLabel || 'a mixed posture'} because timeframe agreement, momentum, and volatility are doing more work than any single headline tonight.`,
      source: 'Midnight context model',
      sourceType: 'note',
      assetMentions: [symbol],
      relevanceScore: 3.6,
      sentimentHint: asset.sentiment === 'bearish' ? 'cautious' : 'mixed',
      matchLabel: 'Signal-driven catalyst',
    },
    {
      id: `${symbol}-watch-next`,
      catalystType: 'Altcoin',
      title: `Watch for confirmation around ${symbol}`,
      body: `The next useful catalyst is whether ${symbol} keeps follow-through on the 15m and 1h alignment instead of fading back into the broader pack.`,
      source: 'Watch next engine',
      sourceType: 'note',
      assetMentions: [symbol],
      relevanceScore: 2.9,
      sentimentHint: 'mixed',
      matchLabel: 'Forward-looking watch',
    },
  ];
}

function buildCatalystFeed(asset = {}, rankedAssets = [], items = [], regimeSummary = {}) {
  if (!items.length) return buildFallbackCatalystFeed(asset, rankedAssets, regimeSummary);

  const feed = items
    .map((item) => {
      const mentions = Array.isArray(item.assetMentions) ? item.assetMentions : [];
      const direct = mentions.includes(asset.symbol);
      const topicBucket = item.topicBucket || item.catalystType || 'Market';
      const sourceNetwork = item.sourceNetwork || (item.sourceType === 'x' ? 'x' : item.sourceType === 'article' ? 'rss' : item.sourceType === 'newsletter' ? 'newsletter' : 'manual');
      return {
        id: item.id,
        catalystType: item.catalystType || 'Market',
        topicBucket,
        title: item.title,
        body: summarizeCatalystBody(item),
        source: item.source,
        sourceType: item.sourceType,
        sourceNetwork,
        ...
        sentimentHint: item.sentimentHint || 'mixed',
        matchTier: item.matchTier || (direct ? 'Direct asset match' : mentions.length ? 'Cross-asset match' : ['Macro', 'Market'].includes(topicBucket) ? 'Market-wide context' : 'General context'),
        matchLabel: direct ? 'Direct context match' : mentions.length ? `Cross-asset read · ${mentions.join(', ')}` : 'Broader market context',
        timestamp: item.timestamp || null,
        freshnessLabel: formatFreshness(item.timestamp || null),
      };
    })
...
function buildCatalystMatch(asset = {}, feed = []) {
  const top = feed[0];
  if (!top) {
    return {
      label: 'No clear catalyst detected',
      detail: `${asset.symbol || 'This asset'} is still being led more by internal signal posture than by any single external driver right now.`,
      strength: 'Signal-led',
    };
  }

  const direct = (top.assetMentions || []).includes(asset.symbol);
  const score = Number(top.relevanceScore || 0);
  const label = direct ? 'Possible catalyst match' : 'Broader context match';
  const strength = score >= 5 ? 'High relevance' : score >= 3 ? 'Moderate relevance' : 'Light relevance';
  const detail = direct
    ? `${top.title} is the strongest live context match for ${asset.symbol}. Treat it as supporting context, not a substitute for confirmation.`
    : `${top.title} is shaping the broader tape around ${asset.symbol}, even if it is not a direct one-symbol catalyst.`;

  return { label, detail, strength };
}

function buildCatalystBridge(feed = []) {
  const counts = feed.reduce((acc, item) => {
    const key = item.catalystType || 'Market';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});

  const sourceCounts = feed.reduce((acc, item) => {
    const key = item.sourceNetwork || (item.sourceType === 'x' ? 'x' : item.sourceType === 'article' ? 'rss' : item.sourceType === 'newsletter' ? 'newsletter' : 'manual');
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});

  const topicCounts = feed.reduce((acc, item) => {
    const key = item.topicBucket || item.catalystType || 'General';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});

  return {
    source: 'Cardano Midnight News bridge armed',
    detail: 'The context layer now accepts X posts, RSS/articles, newsletters, manual notes, and nightly brief items in one normalized schema so the app can fuse catalysts without building a noisy feed.',
    counts,
    sourceCounts,
    topicCounts,
    accepts: ['x', 'rss', 'manual', 'newsletter', 'nightly'],
    fields: ['headline/title', 'summary/body', 'url', 'assetMentions', 'sentimentHint', 'publishedAt', 'sourceHandle', 'sourceNetwork', 'topicBucket', 'externalId', 'imageUrl'],
  };
}

function buildRelatedSignalItems(asset = {}, feed = []) {
  return (feed || []).slice(0, 3).map((item) => ({
    id: item.id,
    title: item.title,
    relation: (item.assetMentions || []).includes(asset.symbol) ? 'Related to this signal' : 'Broader tape context',
    tone: sentimentLabel(item.sentimentHint || 'mixed'),
    source: item.source,
    sourceHandle: item.sourceHandle || '',
    sourceNetwork: item.sourceNetwork || 'manual',
    topicBucket: item.topicBucket || 'General',
    matchTier: item.matchTier || 'General context',
    url: item.url || '',
  }));
}
...
export function buildSignalContext(asset, rankedAssets = [], watchlist = [], regimeSummary = {}, liveContext = {}) {
  ...
  const catalystFeed = buildCatalystFeed(asset, rankedAssets, items, regimeSummary);
  const catalystMatch = buildCatalystMatch(asset, catalystFeed);
  const catalystBridge = buildCatalystBridge(catalystFeed);
  const relatedSignalItems = buildRelatedSignalItems(asset, catalystFeed);
  const nightlyBrief = buildNightlyBrief(asset, liveContext, catalystFeed);
