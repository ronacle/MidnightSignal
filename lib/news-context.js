  ...
  const feed = items
    .map((item) => {
      ...
      return {
        ...
        source: item.source,
        sourceType: item.sourceType,
        sourceNetwork: item.sourceNetwork || (item.sourceType === 'x' ? 'x' : item.sourceType === 'article' ? 'rss' : 'manual'),
...
function buildCatalystBridge(feed = []) {
  ...
  const sourceCounts = feed.reduce((acc, item) => {
    const key = item.sourceNetwork || (item.sourceType === 'x' ? 'x' : item.sourceType === 'article' ? 'rss' : 'manual');
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});
  ...
  return {
    ...
    detail: 'The context layer now accepts X posts, RSS/articles, and manual notes in one normalized schema so the app can fuse catalysts without building a noisy feed.',
    counts,
    sourceCounts,
    accepts: ['x', 'rss', 'manual'],
    fields: ['headline/title', 'summary/body', 'url', 'assetMentions', 'sentimentHint', 'publishedAt'],
  };
}
