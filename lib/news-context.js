...
function buildCatalystBridge(feed = []) {
  ...
  return {
    ...
    detail: 'The context layer now accepts X posts, RSS/articles, newsletters, and manual notes in one normalized schema so the app can fuse catalysts without building a noisy feed.',
    counts,
    sourceCounts,
    topicCounts,
    accepts: ['x', 'rss', 'manual', 'newsletter'],
    fields: ['headline/title', 'summary/body', 'url', 'assetMentions', 'sentimentHint', 'publishedAt', 'topicBucket', 'externalId', 'imageUrl'],
  };
}
