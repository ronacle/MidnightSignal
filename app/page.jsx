...
const BUILD_VERSION = "11.10.3";
const BUILD_LABEL = "exact shell restore + sync graft";
...
function cloudToLocalStrategy(value) {
  const normalized = String(value || "Swing").toLowerCase();
  return normalized === "position" ? "position" : normalized === "scalp" ? "scalp" : "swing";
}

function cloudToLocalTimeframe(value) {
  const normalized = String(value || "1H").toUpperCase();
  if (normalized === "15M") return "7";
  if (normalized === "4H") return "90";
  return "30";
}

function localToCloudStrategy(value) {
  const normalized = String(value || "swing").toLowerCase();
  return normalized === "position" ? "Position" : normalized === "scalp" ? "Scalp" : "Swing";
}

function localToCloudTimeframe(value) {
  if (String(value) === "7") return "15M";
  if (String(value) === "90") return "4H";
  return "1H";
}
...
export default function Page(){
  ...
  const syncPushedRef = useRef(false);
        ...
        <section className="ms-card">
          <div className="ms-row">
            ...
            <div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"center",justifyContent:"flex-end"}}>
              <div style={{padding:"10px 14px",borderRadius:14,border:"1px solid rgba(139,168,255,.18)",background:"rgba(96,103,249,.10)",fontSize:12,fontWeight:700,color:"#dbe8ff"}}>{user ? (syncing ? "Syncing…" : "Cloud ready") : "Saved locally"}</div>
                ...
                <div className="panel-group">
                  <section className="panel-section">
                    <div className="panel-section-head">
                      <div>
                        <div className="panel-kicker">Account Sync</div>
                        <div className="panel-title">Cross-device state</div>
                        <div className="panel-note">Keep your mode, watchlist, selected asset, and disclaimer acceptance following you across devices.</div>
                      </div>
                      <div className="panel-pill">{user ? "Cloud ready" : "Local mode"}</div>
                    </div>
                    <div style={{display:"grid",gap:14}}>
                      <div className="panel-grid-2">
                        <div style={{padding:12,borderRadius:14,background:"rgba(247,247,247,.03)",border:"1px solid rgba(247,247,247,.08)"}}>
                          <div style={{fontSize:13,color:"#94a3b8",marginBottom:6}}>Status</div>
                          <div style={{fontWeight:700}}>{syncStatus}</div>
                        </div>
                        <div style={{padding:12,borderRadius:14,background:"rgba(247,247,247,.03)",border:"1px solid rgba(247,247,247,.08)"}}>
                          <div style={{fontSize:13,color:"#94a3b8",marginBottom:6}}>Last sync</div>
                          <div style={{fontWeight:700}}>{lastSyncedAt ? new Date(lastSyncedAt).toLocaleString() : "Not yet synced"}</div>
                        </div>
                      </div>
                      {!supabaseReady ? (
                        <div className="ms-sub">Add <code>NEXT_PUBLIC_SUPABASE_URL</code> and <code>NEXT_PUBLIC_SUPABASE_ANON_KEY</code> in Vercel to enable account sync.</div>
                      ) : user ? (
                        <div style={{display:"grid",gap:12}}>
                          <div className="ms-sub">Signed in as <b>{user.email}</b>. Current build syncs your shell preferences quietly underneath the restored UI.</div>
                          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                            <button type="button" className="btn-strong" style={{width:"auto"}} onClick={refreshFromCloud} disabled={syncing}>{syncing ? "Syncing..." : "Sync now"}</button>
                            <button type="button" className="btn" style={{width:"auto"}} onClick={signOut}>Sign out</button>
                          </div>
                        </div>
                      ) : (
                        <div style={{display:"grid",gap:12}}>
                          <label>
                            <div style={{fontSize:13,color:"#94a3b8",marginBottom:8}}>Email for magic link</div>
                            <input className="select" type="email" placeholder="you@example.com" value={authEmail} onChange={(e)=>setAuthEmail(e.target.value)} />
                          </label>
                          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                            <button
                              type="button"
                              ...
                              style={{width:"auto"}}
                              onClick={async ()=>{
                                const result = await signInWithEmail(authEmail);
                                setAuthMessage(result?.error ? (result.error.message || "Unable to send magic link") : "Magic link sent");
                              }}
                            >
                              Send magic link
                            </button>
                          </div>
                          {authMessage ? <div className="ms-sub">{authMessage}</div> : null}
                        </div>
                      )}
                    </div>
                  </section>
