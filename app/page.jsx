...
const BUILD_LABEL = "micro refinement";
...
function convictionTier(confidence){ if(confidence >= 75) return "Strong"; if(confidence >= 60) return "Moderate"; return "Light"; }
...
export default function Page(){
  ...
  const topConviction = topSignal ? convictionTier(topSignal.confidence) : "";
  const activeConviction = active ? convictionTier(active.confidence) : "";
  ...
  return (
    <main style={{minHeight:"100vh",color:"#f7f7f7",background:"radial-gradient(circle at top, rgba(42,107,255,.14), transparent 28%), linear-gradient(135deg, #0d1530 0%, #181c2f 45%, #0f1330 100%)",padding:"24px 0 40px"}}>
      <style>{`
        .ms-wrap{max-width:1360px;margin:0 auto;padding:0 24px;display:grid;gap:22px}
        .ms-card{background:linear-gradient(180deg, rgba(24,28,47,.86), rgba(17,24,39,.82));border:1px solid rgba(247,247,247,.09);border-radius:28px;box-shadow:0 18px 70px rgba(0,0,0,.32);backdrop-filter:blur(18px);padding:24px}
        .ms-grid{display:grid;gap:20px}.ms-hero{grid-template-columns:minmax(0,1.45fr) minmax(320px,.95fr)}.ms-stats{grid-template-columns:repeat(4,minmax(0,1fr))}.ms-watch{grid-template-columns:repeat(3,minmax(0,1fr))}.ms-coins{grid-template-columns:repeat(4,minmax(0,1fr))}.ms-context{grid-template-columns:minmax(0,1.05fr) minmax(300px,.95fr)}
        .ms-row{display:flex;gap:14px;align-items:center;justify-content:space-between}.ms-title{font-size:34px;font-weight:900;letter-spacing:-.035em;line-height:1.02;margin:0}.ms-sub{font-size:14px;color:rgba(247,247,247,.68);line-height:1.65}
        .ms-metric{border:1px solid rgba(247,247,247,.08);background:linear-gradient(180deg, rgba(247,247,247,.045), rgba(247,247,247,.025));border-radius:20px;padding:16px;box-shadow:inset 0 1px 0 rgba(255,255,255,.03)}.ms-metric-label{font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:rgba(247,247,247,.58)}.ms-metric-value{margin-top:8px;font-size:24px;font-weight:800;letter-spacing:-.02em}
        .coin-btn{border:1px solid rgba(247,247,247,.08);background:linear-gradient(180deg, rgba(24,28,47,.82), rgba(15,19,36,.82));border-radius:24px;padding:18px;box-shadow:0 14px 50px rgba(0,0,0,.28);display:grid;gap:12px;text-align:left;color:#fff;transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease,opacity .18s ease,background .18s ease;cursor:pointer;min-height:248px}
        .coin-btn:hover{transform:translateY(-3px);box-shadow:0 20px 56px rgba(0,0,0,.34),0 0 0 1px rgba(139,168,255,.14);border-color:rgba(139,168,255,.28);background:linear-gradient(180deg, rgba(30,35,58,.88), rgba(17,24,39,.86))} .coin-btn.active{border-color:rgba(139,168,255,.65);box-shadow:0 0 0 2px rgba(139,168,255,.18),0 14px 50px rgba(0,0,0,.3)} .coin-btn.dim{opacity:.78;filter:saturate(.88)}
        .watch-card{border:1px solid rgba(96,103,249,.28);background:linear-gradient(135deg, rgba(96,103,249,.15), rgba(0,51,173,.16));border-radius:24px;padding:18px;text-align:left;color:#fff;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease,background .18s ease;min-height:210px}.watch-card:hover{transform:translateY(-2px);box-shadow:0 16px 44px rgba(0,0,0,.26);background:linear-gradient(135deg, rgba(96,103,249,.18), rgba(0,51,173,.2))}.watch-card.priority{border-color:rgba(0,255,157,.28);box-shadow:0 0 0 1px rgba(0,255,157,.1), 0 16px 44px rgba(0,0,0,.26)}
        .top-signal-shell{position:relative;overflow:hidden;border-color:rgba(139,168,255,.2);box-shadow:0 0 0 1px rgba(139,168,255,.08),0 24px 82px rgba(0,0,0,.38)}.top-signal-shell::after{content:"";position:absolute;inset:-120px;pointer-events:none;background:radial-gradient(circle, rgba(139,168,255,.10) 0%, rgba(139,168,255,0) 58%);opacity:.62}.top-signal-shell::before{content:"";position:absolute;inset:18% 30%;border-radius:999px;border:1px solid rgba(139,168,255,.22);box-shadow:0 0 0 16px rgba(139,168,255,.035),0 0 0 34px rgba(139,168,255,.02);opacity:.78;pointer-events:none}.top-signal-shell.signal-pulse{animation:signalPulse .9s ease-in-out;box-shadow:0 0 0 1px rgba(139,168,255,.22),0 28px 92px rgba(0,0,0,.42)}
...
.asset-overlay{position:fixed;inset:0;background:rgba(0,0,0,.42);z-index:57}.asset-panel{position:fixed;top:0;right:0;height:100vh;width:min(520px,96vw);z-index:58;background:rgba(11,18,39,.98);border-left:1px solid rgba(247,247,247,.1);backdrop-filter:blur(14px);box-shadow:-18px 0 60px rgba(0,0,0,.4);display:grid;grid-template-rows:auto 1fr}.asset-panel-body{overflow:auto;padding:20px 20px 30px}.asset-sheet-handle{width:56px;height:6px;border-radius:999px;background:rgba(247,247,247,.18);margin:0 auto 12px}.asset-clickable{cursor:pointer}.panel-section{padding:18px;border-radius:22px;border:1px solid rgba(247,247,247,.08);background:rgba(255,255,255,.03)}.panel-section-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:14px}.panel-kicker{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:#8ba8ff;margin-bottom:6px}.panel-title{font-size:18px;font-weight:800}.panel-note{font-size:13px;color:#94a3b8;line-height:1.7;max-width:320px}.panel-pill{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:7px 10px;border:1px solid rgba(139,168,255,.2);background:rgba(96,103,249,.12);color:#dbe8ff;font-size:11px;font-weight:800}.panel-group{display:grid;gap:18px}.panel-grid-2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}@media (max-width:700px){.asset-panel{top:auto;right:0;left:0;bottom:0;height:min(78vh,720px);width:100%;border-left:0;border-top:1px solid rgba(247,247,247,.1);border-top-left-radius:24px;border-top-right-radius:24px;box-shadow:0 -18px 60px rgba(0,0,0,.45)} .learn-fab{right:14px;bottom:14px} .panel-grid-2{grid-template-columns:1fr}}
        @media (max-width:1100px){.ms-hero,.ms-stats,.ms-watch,.ms-coins,.ms-context{grid-template-columns:repeat(2,minmax(0,1fr))}} .hero-card{padding:26px 24px}.hero-lockup{display:grid;grid-template-columns:auto 1fr;gap:18px;align-items:center}.hero-actions{display:flex;gap:12px;flex-wrap:wrap;align-items:center;justify-content:flex-end}.hero-brand{font-size:11px;letter-spacing:.16em;text-transform:uppercase;color:#8ba8ff;font-weight:800;margin-bottom:8px}.hero-app{font-size:38px;font-weight:900;letter-spacing:-.04em;line-height:.96;margin:0 0 8px}.hero-tagline{margin-top:10px;max-width:680px;font-size:15px;line-height:1.65;color:#dbe8ff}.hero-tagline-secondary{margin-top:8px;font-size:13px;line-height:1.55;color:#9fb1d9}.hero-pills{margin-top:14px;display:flex;gap:8px;flex-wrap:wrap}.opening-read-card{padding:18px 22px;border-radius:24px;background:linear-gradient(135deg, rgba(96,103,249,.12), rgba(13,21,48,.78));border:1px solid rgba(139,168,255,.18)}.opening-kicker{font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:#bcd0ff;margin-bottom:8px}.opening-head{font-size:28px;font-weight:900;line-height:1.05;margin-bottom:8px} @media (max-width:700px){.ms-wrap{padding:0 14px;gap:16px}.ms-card{padding:18px;border-radius:24px}.ms-hero,.ms-stats,.ms-watch,.ms-coins,.ms-context{grid-template-columns:1fr}.ms-row{flex-direction:column;align-items:stretch}.ms-title{font-size:28px}.ms-sub{font-size:13px}.focus-chip{font-size:28px;padding:16px 18px;width:100%}.coin-btn,.watch-card{min-height:auto}.top-signal-shell{position:relative;top:auto}.alert-toast{left:14px;right:14px;bottom:14px;min-width:0;max-width:none}.btn,.select,.btn-strong{min-height:46px}.asset-panel-body{padding:18px 16px 26px}.hero-card{padding:20px 18px}.hero-lockup{grid-template-columns:1fr;justify-items:center;text-align:center;gap:12px}.hero-copy{display:grid;justify-items:center;gap:0}.hero-brand{margin-bottom:6px}.hero-app{font-size:34px}.hero-tagline{font-size:14px;max-width:32ch}.hero-tagline-secondary{font-size:12px;max-width:32ch}.hero-actions{justify-content:stretch;margin-top:2px}.hero-actions .btn,.hero-actions .btn-strong{width:100%}.hero-pills{justify-content:center;margin-top:12px}.opening-read-card{margin-top:2px}.opening-head{font-size:24px}} @media (max-width:520px){.ms-wrap{padding:0 12px}.ms-card{padding:16px;border-radius:22px}.ms-metric{padding:14px}.ms-metric-value{font-size:22px}.coin-btn{padding:16px;border-radius:20px}.watch-card{padding:16px;border-radius:20px}.hero-app{font-size:32px}.hero-tagline{font-size:13px;line-height:1.6}.hero-tagline-secondary{font-size:11.5px}}
      `}</style>
      ...
      <div className="ms-wrap">
        <section className="ms-card hero-card">
          <div className="ms-row" style={{alignItems:"center"}}>
            <div className="hero-lockup" style={{flex:1,minWidth:0}}>
              <div style={{ flex: "0 0 auto" }}>
                <BeaconLogo size={118} animated={true} />
              </div>
              <div className="hero-copy">
                <div className="hero-brand">Nightly market intelligence</div>
                ...
                <div className="hero-tagline">Transforming Market Data → Information → Knowledge → Understanding → Market Wisdom.</div>
                <div className="hero-tagline-secondary">Know the setup, the why, and act with more confidence.</div>
        ...
        {ritualLoop ? <section className="opening-read-card">
          <div className="opening-kicker">Tonight's opening read</div>
          <div className="opening-head">{ritualLoop.opening}</div>
          <div className="ms-sub" style={{fontSize:15,color:"#e2e8f0"}}>{ritualLoop.momentum}</div>
        </section> : null}

        <section className="ms-card" style={{padding:18,position:"sticky",top:12,zIndex:9,backdropFilter:"blur(10px)",background:"rgba(5,10,24,0.82)",border:"1px solid rgba(139,168,255,.14)"}}>
          <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",flexWrap:"wrap"}}>
            <div>
              <div style={{fontSize:11,letterSpacing:".12em",textTransform:"uppercase",color:"#8ba8ff",fontWeight:800,marginBottom:6}}>Page guide</div>
              <div className="ms-sub" style={{color:"#dbe8ff"}}>Jump to the part of the dashboard you want without losing the nightly thread.</div>
            </div>
        ...
        <section id="flow-section" className="ms-card" style={{scrollMarginTop:96}}>
          <div style={{fontSize:11,letterSpacing:".14em",textTransform:"uppercase",color:"#8ba8ff",fontWeight:800,marginBottom:12}}>Flow</div>
          <div className="ms-row">
            <div>
        ...
        {topSignal && (
          <section id="signal-section" className="ms-grid ms-hero" style={{scrollMarginTop:96}}>
            <div className={`ms-card top-signal-shell ${topPulseOn ? "signal-pulse" : ""}`}>
              <div style={{fontSize:11,letterSpacing:".14em",textTransform:"uppercase",color:"#8ba8ff",fontWeight:800,marginBottom:12}}>Signal</div>
              ...
              <div className="ms-row">
                <div>
                  <div style={{fontSize:11,letterSpacing:".12em",textTransform:"uppercase",color:"#8ba8ff",fontWeight:800}}>Tonight's lead read</div>
                  <div style={{fontSize:14,color:"#94a3b8",marginTop:6}}>
                    Primary focus tonight
                  ...
                  <div className="ms-sub" style={{marginTop:8}}>
                    Tonight's lead setup • {strategy} posture • {timeframe}D context • {dataSource === "coingecko" ? "live market input" : "seed fallback"}
                  </div>
                </div>
                <div
              ...
              <div style={{marginTop:18,padding:18,borderRadius:18,background:"rgba(2,6,23,0.55)",border:"1px solid rgba(148,163,184,0.12)"}}>
                ...
                <div style={{marginTop:14,padding:12,borderRadius:14,background:"linear-gradient(135deg, rgba(96,103,249,.10), rgba(0,51,173,.12))",border:"1px solid rgba(96,103,249,.2)",fontSize:13,color:"#dbe8ff"}}>
                  Clarity pass: this lead card stays visually dominant so the nightly read is obvious at a glance.
                </div>
        ...
        {visitDelta && <section id="history-section" className="ms-card" style={{scrollMarginTop:96}}>
          <div style={{fontSize:11,letterSpacing:".14em",textTransform:"uppercase",color:"#8ba8ff",fontWeight:800,marginBottom:12}}>History</div>
          <div className="ms-row">
            <div>
        ...
        <section id="watchlist-section" className="ms-card" style={{scrollMarginTop:96}}><div style={{fontSize:11,letterSpacing:".14em",textTransform:"uppercase",color:"#8ba8ff",fontWeight:800,marginBottom:12}}>Watchlist</div><div className="ms-row"><div><div style={{fontSize:20,fontWeight:700}}>Watchlist</div><div className="ms-sub">Your pinned Midnight Signal picks, kept close to the nightly read.</div></div><div style={{fontSize:12,color:"rgba(247,247,247,.5)"}}>{watchlist.length} tracked</div></div><div className="ms-sub" style={{marginTop:10}}>Watchlist names now sit visually above the field with a brighter priority treatment when a signal shifts.</div><div className="ms-grid ms-watch" style={{marginTop:14}}>{ordered.filter(c=>watchlist.includes(c.symbol)).map(coin => <button className={`watch-card priority ${watchPulseSymbol===coin.symbol ? "watch-pulse" : ""}`} key={coin.symbol} onClick={()=>openAsset(coin.symbol, "momentum")}><div className="ms-row"><div><div style={{fontSize:20,fontWeight:700}}>{coin.symbol}</div><div className="ms-sub">{coin.name}</div></div><div style={{display:"grid",justifyItems:"end",gap:6}}><Pill>{coin.posture}</Pill><div style={{fontSize:11,color:"rgba(247,247,247,.62)",fontWeight:700,letterSpacing:".04em"}}>{convictionTier(coin.confidence)} conviction</div></div></div><div style={{fontSize:28,fontWeight:700,marginTop:10}}>{formatPrice(coin.price)}</div><div style={{display:"grid",gap:4}}><div style={{fontSize:12,color:"#94a3b8"}}>24h Change</div><div style={{fontSize:14,fontWeight:700,color:coin.change24h>=0?"#22c55e":"#3b82f6",display:"flex",alignItems:"center",gap:4}}>{coin.change24h>=0?"+":""}{coin.change24h.toFixed(2)}%<span style={{fontSize:12}}>{coin.change24h>=0?"↑":"↓"}</span></div></div><div style={{marginTop:12}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:12,color:"rgba(247,247,247,.5)"}}><span>{timeframe}D derived sparkline</span><span>{coin.confidence}%</span></div>{sparkline(coin.history, coin.change24h>=0)}</div></button>)}</div></section>

        <section id="market-section" className="ms-card" style={{scrollMarginTop:96}}><div style={{fontSize:11,letterSpacing:".14em",textTransform:"uppercase",color:"#8ba8ff",fontWeight:800,marginBottom:12}}>Market View</div><div className="ms-row"><div><div style={{display:"flex",gap:8,alignItems:"center"}}><h2 style={{fontSize:24,margin:0}}>Top 20 Opportunity Grid</h2>{mode==="Beginner"?<span style={{fontSize:12,color:"rgba(247,247,247,.5)"}}>ⓘ Higher scores = stronger alignment, not certainty.</span>:null}</div><div className="ms-sub">Tap a card for the deeper read. Non-leaders stay visually quieter so tonight’s focus stays obvious.</div></div></div><div className="ms-grid ms-coins" style={{marginTop:16}}>{ordered.map(coin => <button key={coin.symbol} className={`coin-btn ${active?.symbol===coin.symbol?"active":""} ${topSignal?.symbol!==coin.symbol?"dim":""}`} style={watchlist.includes(coin.symbol) ? {borderColor:"rgba(0,255,157,.22)", boxShadow:"0 0 0 1px rgba(0,255,157,.08), 0 14px 50px rgba(0,0,0,.32)"} : undefined} onClick={()=>openAsset(coin.symbol, "confidence")}><div className="ms-row"><button type="button" onClick={(e)=>{e.stopPropagation();toggleWatch(coin.symbol);}} className={`watch-star ${watchlist.includes(coin.symbol)?"active":""}`} style={{color:watchlist.includes(coin.symbol)?"#00ff9d":"#fff"}}>{watchlist.includes(coin.symbol)?"★":"☆"}</button><div style={{display:"grid",justifyItems:"end",gap:6}}><Pill>{coin.posture}</Pill><div style={{fontSize:11,color:"rgba(247,247,247,.62)",fontWeight:700,letterSpacing:".04em"}}>{convictionTier(coin.confidence)} conviction</div></div></div><div><div style={{fontSize:22,fontWeight:800}}>{coin.symbol}</div><div className="ms-sub">{coin.name}</div></div><div style={{fontSize:28,fontWeight:700}}>{formatPrice(coin.price)}</div><div style={{display:"grid",gap:4}}><div style={{fontSize:12,color:"#94a3b8"}}>24h Change</div><div style={{fontSize:14,fontWeight:700,color:coin.change24h>=0?"#22c55e":"#3b82f6",display:"flex",alignItems:"center",gap:4}}>{coin.change24h>=0?"+":""}{coin.change24h.toFixed(2)}%<span style={{fontSize:12}}>{coin.change24h>=0?"↑":"↓"}</span></div></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Pill tone={coin.timing}>{coin.timing}</Pill></div><div><div className="ms-row" style={{marginBottom:8}}><span style={{fontSize:12,color:"rgba(247,247,247,.5)"}}>Signal Confidence</span><span style={{fontSize:12,color:"rgba(247,247,247,.7)"}}>{coin.confidence}%</span></div><div style={{height:8,width:"100%",borderRadius:999,background:"rgba(247,247,247,.1)",overflow:"hidden"}}><span style={{display:"block",height:"100%",width:`${coin.confidence}%`,borderRadius:999,background:"linear-gradient(90deg, #0033AD, #6067F9, #8BA8FF)"}}></span></div></div></button>)}</div></section>
          ...
          <aside className="asset-panel">
            <div style={{padding:"18px 20px",borderBottom:"1px solid rgba(247,247,247,.08)",display:"flex",justifyContent:"space-between",gap:12,alignItems:"center"}}>
              <div>
                ...
                <div style={{fontSize:28,fontWeight:900,color:activeAccent.color,textShadow:activeAccent.glow}}>{active.symbol} • {active.posture}</div><div style={{fontSize:12,letterSpacing:".08em",textTransform:"uppercase",color:"rgba(247,247,247,.6)",fontWeight:800,marginTop:6}}>{activeConviction} conviction • {active.confidence}% confidence</div>
