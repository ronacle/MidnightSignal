...
function buildComparison(coins){
  ...
  coins.slice(0,5).forEach(c=>{
    const prevVal = prev[c.symbol];
    if(prevVal == null) return;
    const diff = c.signal - prevVal;
    if(diff > 0.02){
      results.push(`${c.symbol} stronger than yesterday`);
    } else if(diff < -0.02){
      results.push(`${c.symbol} weaker than yesterday`);
    } else {
      results.push(`${c.symbol} holding steady`);
    }
  });
...
function buildUserJourneyNarrative(coins){
  if(!Array.isArray(coins) || !coins.length) return "No major changes since your last check — conditions remain steady.";
  ...
  return parts.length ? parts.join(". ") + "." : "No major changes since your last check — conditions remain steady.";
}
...
const compareItems=buildComparison(state.coins);
