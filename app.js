...
function buildUserJourneyNarrative(coins){
  if(!Array.isArray(coins) || !coins.length) return "No major changes since your last visit.";

  const improving = coins.filter(c=>c.timing==="Improving").slice(0,2);
  const weakening = coins.filter(c=>c.timing==="Weakening").slice(0,1);
  const watched = (state.watchlist||[]).slice(0,1).map(sym=>{
    const c = coins.find(x=>x.symbol===sym);
    return c ? `${c.symbol} is ${c.timing==="Improving"?"starting to gain strength":"holding steady"}` : null;
  }).filter(Boolean);

  let parts = [];

  if(watched.length){
    parts.push(watched[0]);
  }

  if(improving.length){
    parts.push(`${improving.map(c=>c.symbol).join(", ")} are showing improving conditions`);
  }

  if(weakening.length){
    parts.push(`${weakening[0].symbol} is showing weaker momentum`);
  }

  return parts.length ? parts.join(". ") + "." : "No major changes since your last visit.";
}

function buildTonightStory(topSignal, context){
  if(!topSignal) return "No strong story is forming right now.";

  const strong = topSignal.signal >= 0.7;
  const improving = topSignal.timing === "Improving";

  let base = `${topSignal.symbol} is showing ${strong ? "a strong" : "a developing"} setup right now.`;

  let market = context?.market || "";
  let ecosystem = context?.ecosystem || "";

  let tone = improving
    ? "Momentum is improving, but still needs confirmation."
    : "The setup may still require confirmation before acting.";

  return `${base} ${market} ${ecosystem} ${tone}`;
}

function buildSignalContext(topSignal){
  if(!topSignal){
    return {
      market:"Market context is unclear right now.",
      ecosystem:"No strong ecosystem signals detected."
    };
  }
  ...
  return {
    market: strong
      ? "Broader market momentum appears supportive, which may be helping strengthen this setup."
      : "Market conditions look more mixed, which can make signals less reliable.",
    ecosystem: bullish
      ? "Positive sentiment around this asset’s ecosystem may be contributing to the current strength."
      : "There is no strong ecosystem push behind this setup right now."
  };
}
...
function buildLearningHelper(kind){
  const beginner = state.learningMode !== "advanced";
  const copy = {
    signal: beginner
      ? "Signal Strength is a shortcut for how strong or weak the current setup looks. Higher usually means the setup is cleaner, not guaranteed."
      : "Signal Strength compresses multiple inputs into a fast read on setup quality and alignment.",
    timing: beginner
      ? "Timing helps answer: does this setup look ready now, still early, or too weak to trust?"
      : "Timing frames whether conditions look actionable, developing, or deteriorating.",
    mtf: beginner
      ? "Multi-timeframe alignment checks whether different chart views are agreeing. Agreement usually makes a setup easier to trust."
      : "MTF alignment shows whether short, medium, and broader views are reinforcing each other.",
    decision: beginner
      ? "Use the decision layer as guidance, not a command. The goal is to help you think clearly, not react blindly."
      : "Decision support converts signal quality into a practical stance while preserving discretion."
  };
  return copy[kind] || "";
}

function getLiveStatusLabel(){
  if(!state.lastUpdated) return "Waiting for market update";
  const mins = Math.max(0, Math.floor((Date.now() - state.lastUpdated.getTime()) / 60000));
  if(mins < 1) return "Updated just now";
  if(mins === 1) return "Updated 1 min ago";
...
<section class="card top-signal fade-in ${signalChanged ? "pulse-glow" : ""} pulse-frame"><div class="caps">Tonight’s Brief</div><div class="story-block"><div class="story-title">Tonight’s Story</div><div class="story-text">${story}</div></div><div class="journey-block"><div class="journey-title">Since your last visit</div><div class="journey-text">${journeyNarrative}</div></div><div class="sound-toggle" id="soundToggle">${state.soundEnabled ? "Sound: ON" : "Sound: OFF"}</div><div class="tiny" style="margin-top:6px">Start here first. This is the clearest current read in the app.</div>${topSignal?`<div style="margin-top:10px;display:flex;gap:10px;align-items:center;flex-wrap:wrap"><div class="title" style="font-size:28px">${topSignal.symbol}</div><span class="signal-label ${topSignal.adaptiveLabel.cls}">${topSignal.adaptiveLabel.title}</span>${badge(topSignal.regime)}${badge(topSignal.timing)}</div><div class="subtitle" style="margin-top:8px">${topSignal.name}</div><div class="grid" style="grid-template-columns:1fr 1fr;margin-top:16px"><div class="mini"><div class="tiny">Signal</div><div style="margin-top:6px;font-weight:700">${Math.round(topSignal.signal*100)}%</div><div class="tiny" style="margin-top:4px">${getConfidenceContext(topSignal.signal)}</div><div class="tiny" style="margin-top:4px">Higher = stronger setup.</div><div class="helper-note">${buildLearningHelper("signal")}</div></div><div class="mini"><div class="tiny">Suggested Posture</div><div style="margin-top:6px">${postureBadge(topSignal)}</div></div></div>
