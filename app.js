...
const COINGECKO_HISTORY_URL = (id, days) =>
...
function calculateSignal(coin) {
  ...
  const shortBias = indicators.maTrend === "Bullish" && indicators.rsi < 70
    ? 1
    : indicators.maTrend === "Bearish" && indicators.rsi > 30
      ? -1
      : 0;
...
function getPostureExplanation(coin) {
  ...
  if (coin.signal >= 0.75 && coin.timing === "Enter" && coin.indicators.maTrend === "Bullish") {
    parts.push("Alignment across signal, timing, and trend supports continuation conditions.");
  } else if (coin.signal >= 0.6 && coin.timing !== "Reduce") {
    parts.push("Moderate alignment; waiting for stronger confirmation can improve probability.");
  } else if (coin.signal < 0.5 || coin.timing === "Reduce" || coin.indicators.maTrend === "Bearish") {
    parts.push("Weak or negative alignment; risk is elevated and conditions are defensive.");
  } else {
    parts.push("Mixed signals; monitoring for clearer structure is reasonable.");
  }
  return parts.join(" • ");
}
...
function enrich(coin, index) {
  ...
  return {
    ...
    confluence: {
      bullish,
      bearish,
      bullishTriggered: bullish >= CONFLUENCE_THRESHOLD,
      bearishTriggered: bearish >= CONFLUENCE_THRESHOLD,
    },
...
function simulate(coins) {
  return coins.map((coin, i) => {
    const nextPrice = Math.max(0.0001, coin.price * (1 + (Math.random() - 0.5) * 0.03));
    const nextChange = Math.max(-12, Math.min(12, coin.change24h + (Math.random() - 0.5) * 1.8));
...
function toCoinGeckoId(symbol) {
  const map = {
    ...
    BNB: "binancecoin", TON: "the-open-network", LINK: "chainlink", LTC: "litecoin",
    BCH: "bitcoin-cash", NEAR: "near", APT: "aptos", ARB: "arbitrum", OP: "optimism",
    FIL: "filecoin", ATOM: "cosmos", TRX: "tron"
  };
...
function recompute() {
  ...
  state.bullishHighAlerts = state.enhanced
    .filter(c => c.signal >= 0.7 && c.confluence.bullish >= 70 && String(c.mtf.label).includes("Bullish"))
    .sort((a,b) => b.opportunityScore - a.opportunityScore)
    .slice(0,4);
  state.bearishHighAlerts = state.enhanced
    .filter(c => (c.signal <= 0.45 || c.timing === "Reduce") && c.confluence.bearish >= 70 && String(c.mtf.label).includes("Bearish"))
    .sort((a,b) => b.confluence.bearish - a.confluence.bearish)
    .slice(0,4);
...
function renderOnboarding() {
    ...
    <div style="position:fixed;inset:0;z-index:50;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.7);backdrop-filter:blur(8px)">
      <div class="card" style="max-width:520px;width:calc(100% - 24px)">
        ...
        <div class="grid" style="grid-template-columns:repeat(3,1fr)">
          ${STRATEGY_OPTIONS.map(s => `<button class="btn-soft" data-strategy="${s}">${s[0].toUpperCase()+s.slice(1)}</button>`).join("")}
        </div>
  ...
  el.querySelectorAll("[data-strategy]").forEach(btn => btn.onclick = () => {
    state.strategy = btn.dataset.strategy;
    storage.set("midnight-html-strategy", state.strategy);
    recompute();
    render();
  });
  el.querySelector("#enterDash").onclick = () => {
    ...
    state.coins = await Promise.all(rows.map(async (row, i) => {
      ...
      if (!priceHistory || priceHistory.length < 10) {
        ...
        if (hist && hist.length) {
          ...
          priceHistory = buildSyntheticHistory({
            price: nextPrice,
...
function startLivePulse() {
  ...
  setInterval(() => {
    state.pulseTick = (state.pulseTick + 1) % 100000;
    render();
  }, 10000);
...
function render() {
  ...
  const searchResults = state.assetQuery
    ? state.enhanced.filter(c => `${c.symbol} ${c.name}`.toLowerCase().includes(state.assetQuery.toLowerCase())).slice(0, 5)
    : [];
  ...
  app.innerHTML = `
    <section class="card fade-in">
      <div class="row">
        ...
        <div style="width:min(420px,100%)">
          ...
          ${searchResults.length ? `<div class="grid" style="margin-top:8px">${searchResults.map(item => `<button class="search-item" data-add="${item.symbol}">${item.symbol} — ${item.name}</button>`).join("")}</div>` : ""}
        </div>
      </div>
    </section>

    ${(state.bullishHighAlerts.length || state.bearishHighAlerts.length) ? `
    <section class="grid grid-alerts fade-in">
      <div class="card">
    ...
    ${state.firstInsight ? `
    <section class="fade-in insight ${state.firstInsight.adaptiveLabel?.priority === 'high' ? 'priority-high' : ''} ${Math.round(state.firstInsight.signal*100) >= 90 ? 'priority-elite' : ''}">
      ...
      <div class="row" style="position:relative">
        <div>
          ...
          <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
            <span class="signal-label ${state.firstInsight.adaptiveLabel.cls}">${state.firstInsight.adaptiveLabel.title}</span>
            <span class="tiny">${state.firstInsight.adaptiveLabel.subtitle}</span>
            ${Math.round(state.firstInsight.signal*100) >= 90 ? `<span class="elite-badge">90+ elite</span>` : ''}
            ${postureBadge(state.firstInsight)}
          </div>
        ...
        <div class="row-start">
          <div>
            ...
            <div class="grid" style="grid-template-columns:repeat(3,minmax(0,1fr));margin-top:14px">
              <div class="mini">
                <div class="tiny">1. Scan</div>
                <div style="margin-top:6px;font-weight:700">Start with alerts + Top 20</div>
                <div class="tiny" style="margin-top:6px">Look for High Probability labels, stronger confidence, and aligned posture.</div>
              </div>
              <div class="mini">
                <div class="tiny">2. Understand</div>
                <div style="margin-top:6px;font-weight:700">Open the detail panel</div>
                <div class="tiny" style="margin-top:6px">Review signal breakdown, confluence, RSI, MA trend, and action insight.</div>
              </div>
              <div class="mini">
                <div class="tiny">3. Decide</div>
                <div style="margin-top:6px;font-weight:700">Use posture, not commands</div>
                <div class="tiny" style="margin-top:6px">Candidate for entry, wait for confirmation, monitor only, or defensive posture.</div>
              </div>
            </div>
            ${state.learnMode ? `<div class="tiny" style="margin-top:10px;color:rgba(247,247,247,.68)">How to think about the app: Signal shows strength, labels show quality, timing shows posture, and confluence shows how many factors agree.</div>` : ""}
          </div>
          <div class="controls">
      ...
      <div class="card">
        ...
        <div class="grid" style="margin-top:14px">
          <label>
            ...
            <select id="strategySelect">${STRATEGY_OPTIONS.map(s => `<option value="${s}" ${state.strategy === s ? 'selected' : ''}>${s[0].toUpperCase()+s.slice(1)}</option>`).join("")}</select>
          </label>
          <label>
            <div class="tiny" style="margin-bottom:6px">Timeframe</div>
            <select id="timeframeSelect">${TIMEFRAME_OPTIONS.map(t => `<option value="${t}" ${state.timeframe === t ? 'selected' : ''}>${t}D</option>`).join("")}</select>
          </label>
          ...
          <div class="mini">
            <div class="tiny">How to use this ${infoBtn('signal')}</div>
            <div style="margin-top:8px;font-size:14px;color:rgba(247,247,247,.76)">
              Start with alerts and Top 20 → open a coin → read Signal Breakdown and Suggested Posture.
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="card fade-in">
      <div class="row-start">
        <div>
          <div class="caps">Your Watchlist</div>
          <div style="font-size:22px;font-weight:700;margin-top:4px">Pinned to the top of your grid</div>
          <div class="subtitle">Starred assets appear first while still respecting the current sort order.</div>
        </div>
        <span class="badge">${state.watchlist.length} starred</span>
      </div>
    </section>` : ""}
    ...
    <section class="fade-in">
      <div class="row-start" style="margin-bottom:10px">
        <div>
          ...
          <div class="subtitle">Adaptive labels, posture, confluence, and signal hierarchy.</div>
        </div>
        <span class="badge">Sorted by ${state.sortBy}</span>
      </div>
      <section class="grid grid-cards">
        ${state.sortedCoins.map(coin => {
          ...
          return `
            <button class="${getCoinClasses(coin, selectedCls)} ${strong ? 'pulse' : ''}" data-select="${coin.symbol}">
              ${strong ? `<div class="shimmer"></div>` : ''}
              <div class="row-start" style="position:relative">
                <button data-watch="${coin.symbol}" style="padding:6px 10px">${state.watchlist.includes(coin.symbol) ? '★' : '☆'}</button>
                <div style="flex:1;min-width:0">
                  ...
                  <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
                    ...
                    <span class="tiny">${coin.adaptiveLabel.subtitle}</span>
                    ${elite ? `<span class="elite-badge">90+ elite</span>` : ''}
                    ${postureBadge(coin)}
                  </div>
                </div>
              ...
              <div>
                ...
                <div class="tiny" style="margin-top:4px;color:rgba(247,247,247,.65)">${getConfidenceContext(coin.signal).label}</div>
              ...
              <div class="grid" style="grid-template-columns:1fr 1fr">
                <div class="mini"><div class="tiny">RSI ${infoBtn('confidence')}</div><div style="margin-top:8px;font-weight:700">${coin.indicators.rsi}</div></div>
                <div class="mini"><div class="tiny">MA Trend ${infoBtn('signal')}</div><div style="margin-top:8px;font-weight:700">${coin.indicators.maTrend}</div></div>
              </div>
    ...
    ${state.selected ? (() => {
      ...
      const elite = Math.round(coin.signal * 100) >= 90;
      return `
        <section class="panel ${coin.adaptiveLabel?.priority === 'high' ? 'detail-priority-high' : coin.adaptiveLabel?.priority === 'low' ? 'detail-priority-low' : ''}">
          <div class="row-start">
            <div>
              <div class="caps">Asset Detail</div>
              <div style="margin-top:6px;display:flex;gap:10px;align-items:center;flex-wrap:wrap">
                <div class="title" style="margin:0">${coin.symbol}</div>
                <span class="signal-label ${coin.adaptiveLabel.cls}">${coin.adaptiveLabel.title}</span>
                ${elite ? `<span class="elite-badge">90+ elite</span>` : ''}
                ${badge(coin.regime)}
                ${badge(coin.timing)}
                ${postureBadge(coin)}
                <button id="glossaryOpenDetail">How to read this</button>
              </div>
              <div class="subtitle" style="margin-top:8px">${coin.name}</div>
            </div>
          </div>

          <div class="detail-grid-6" style="margin-top:18px">
            <div class="mini" style="grid-column: span 6">
              <div class="tiny">Signal Breakdown ${infoBtn('signal')}</div>
              <div style="margin-top:10px;display:grid;gap:10px">
                ${['momentum','trend','volatility','rsi','trendStrength','mtf'].map(key => `
                  <div>
                    <div class="row"><span class="tiny">${key === 'mtf' ? `MTF ${infoBtn('mtf')}` : key === 'rsi' ? `RSI ${infoBtn('confidence')}` : key === 'trendStrength' ? `trend strength ${infoBtn('signal')}` : key}</span><span>${coin.signalBreakdown?.[key] ?? '-'}%</span></div>
                    <div class="progress"><span style="width:${coin.signalBreakdown?.[key] ?? 0}%"></span></div>
                  </div>
                `).join('')}
              </div>
            </div>
          ...
          <div class="detail-grid-half" style="margin-top:20px">
            <div class="mini">
              <div class="tiny">Why this coin stands out</div>
              <div style="margin-top:10px;font-size:14px;color:rgba(247,247,247,.75)">
                ${coin.reasons.join(' • ')}
              </div>
            </div>
            <div class="mini">
              <div class="tiny">Action Insight</div>
              <div style="margin-top:10px;font-size:14px;color:rgba(247,247,247,.75)">
                ${coin.actionInsight.action}
              </div>
            </div>
          </div>
          ...
          <div class="detail-grid-half" style="margin-top:20px">
            <div class="mini">
              <div class="tiny">Bullish Confluence ${infoBtn('confluence')}</div>
              <div style="margin-top:8px;font-weight:700;color:var(--blue3)">${coin.confluence.bullish}/100</div>
            </div>
            <div class="mini">
              <div class="tiny">Bearish Confluence ${infoBtn('confluence')}</div>
              <div style="margin-top:8px;font-weight:700;color:var(--bear)">${coin.confluence.bearish}/100</div>
            </div>
          </div>
        </section>
      `;
        ...
        <section class="card" style="position:fixed;inset:24px;z-index:40;max-width:860px;margin:auto;height:max-content;max-height:calc(100vh - 48px);overflow:auto;background:rgba(16,19,40,.98)">
          <div class="row-start">
            <div>
              ...
              <div class="title" style="font-size:28px;margin-top:6px">${topic.title}</div>
              <div class="subtitle" style="margin-top:8px">Plain-English explanations for beginners and fast reference for everyone else.</div>
            </div>
            <button id="glossaryClose">Close</button>
          </div>
          <div class="grid" style="grid-template-columns:260px 1fr;margin-top:18px">
            <div class="mini" style="display:grid;gap:8px;align-content:start">
              ${Object.entries(GLOSSARY).map(([key, item]) => `<button class="btn-soft" data-topic="${key}" style="text-align:left;${state.glossaryTopic === key ? 'border-color:rgba(139,168,255,.35);background:rgba(139,168,255,.12)' : ''}">${item.title}</button>`).join('')}
            </div>
            <div class="mini">
              <div style="font-size:15px;line-height:1.7;color:rgba(247,247,247,.86)">${topic.body}</div>
              <div class="tiny" style="margin-top:14px">FAQ: Enter does not mean guaranteed buy, Reduce does not mean sell everything, and High Probability does not mean guaranteed win.</div>
            </div>
          </div>
        </section>
    ...
    <section style="margin-top:20px;text-align:center">
      <div class="tiny" style="color:rgba(247,247,247,.5)">
        Midnight Signal • Educational only • Not financial advice • Built for signal discovery and interpretation
      </div>
    </section>
  `;
  ...
  app.querySelectorAll("[data-add]").forEach(btn => btn.addEventListener("click", () => {
    state.selected = btn.dataset.add;
    state.assetQuery = "";
    render();
  }));
  app.querySelectorAll("[data-select]").forEach(btn => btn.addEventListener("click", () => {
  ...
  app.querySelectorAll("[data-glossary]").forEach(btn => btn.addEventListener("click", (e) => {
    e.stopPropagation();
    state.glossaryOpen = true;
    state.glossaryTopic = btn.dataset.glossary;
    render();
  }));
  ...
  if (glossaryOpenDetail) glossaryOpenDetail.addEventListener("click", () => {
    state.glossaryOpen = true;
    state.glossaryTopic = "signal";
    render();
  });
  app.querySelectorAll("[data-topic]").forEach(btn => btn.addEventListener("click", () => {
    state.glossaryTopic = btn.dataset.topic;
    render();
  }));
  const glossaryClose = app.querySelector("#glossaryClose");
  if (glossaryClose) glossaryClose.addEventListener("click", () => {
    state.glossaryOpen = false;
    render();
  });
  const glossaryBackdrop = app.querySelector("#glossaryBackdrop");
  if (glossaryBackdrop) glossaryBackdrop.addEventListener("click", () => {
    state.glossaryOpen = false;
    render();
  });
